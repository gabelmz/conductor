  ...
  for (const c of candidates) {
    if (fs.existsSync(c))
      return {
        python: c,
        appRoot:
          fs.existsSync(path.join(appRoot, "backend")) ? appRoot : devRoot,
      };
  }
  return null;
}
...
function loadStatus(msg) {
  try {
    win?.webContents.send("load:status", msg);
  } catch {
    /* window gone */
  }
}
...
async function startBackend() {
  ...
  const logStream = fs.createWriteStream(path.join(dataDir, "backend.log"), {
    flags: "a",
  });
  ...
  while (Date.now() < deadline) {
    ...
    if (stage === 0 && elapsed > 8000) {
      stage = 1;
      loadStatus("Backend is still warming up…");
    } else if (stage === 1 && elapsed > 25000) {
      stage = 2;
      loadStatus(
        "First launch can take a bit — files are being unpacked and scanned. Hang tight…",
      );
    } else if (stage === 2 && elapsed > 60000) {
      stage = 3;
      loadStatus("Almost there — starting the Python server…");
    }
    await sleep(300);
  }
...
function backendDataDir() {
  ...
  const root =
    fs.existsSync(path.join(appRoot, "backend")) ? appRoot
    : fs.existsSync(path.join(devRoot, "backend")) ? devRoot
    : fs.existsSync(path.join(resources, "backend")) ? resources
    : appRoot;
...
function readKeyFile() {
  try {
    const raw = fs.readFileSync(
      path.join(backendDataDir(), "provider-keys.json"),
      "utf-8",
    );
...
ipcMain.handle("keys:set", (_e, providerId, apiKey) => {
  ...
  try {
    ...
    /* fall back to plain base64 (LAW's documented fallback) */
  }
...
ipcMain.handle("keys:get", (_e, providerId) => {
  ...
  try {
    ...
    return null;
  } // written on another machine/user — report "no key"
});

ipcMain.handle("keys:has", (_e, providerId) =>
  Boolean(readKeyFile()[String(providerId)]),
);
...
function readUpdateToken() {
  ...
  if (process.env.CONDUCTOR_GH_TOKEN)
    return process.env.CONDUCTOR_GH_TOKEN.trim();
  try {
    const t = fs
      .readFileSync(path.join(process.resourcesPath, "gh-token"), "utf8")
      .trim();
    return t || null;
  } catch {
    return null;
...
function setupAutoUpdater() {
  ...
  const emit = (event, data) => {
    try {
      win?.webContents.send("updates:event", { event, ...(data || {}) });
    } catch {
      /* window gone */
    }
  };
  ...
  autoUpdater.on("update-not-available", (info) =>
    emit("update-not-available", { version: info && info.version }),
  );
  ...
  try {
    ...
    /* keep electron-builder publish defaults */
  }
