...
const VIEW_RENDERERS = {
  ...
  bernie: () => renderBernie(),
  asanarules: () =>
    window.ConductorAsanaRules ?
      window.ConductorAsanaRules.render()
    : renderModuleStub("asanarules"),
...
function showView(name) {
  ...
  if (
    ...
    window.ConductorSplit.isActive() &&
    name !== "bernie" &&
    name !== "chat"
  ) {
...
function wireShell() {
  ...
  $("#sidebar-scroll").addEventListener("click", (e) => {
    ...
    if (view.startsWith("url:"))
      window.open(view.slice(4), "_blank", "noopener");
    else showView(view);
  });
  ...
  if (d) {
    ...
    ["btn-min", "btn-max", "btn-close"].forEach((id) =>
      $(`#${id}`).addEventListener("click", () =>
        toast("Window controls are desktop-only", "warn"),
      ),
    );
  }
  ...
  $$("#settings-nav .settings-nav-item").forEach((b) =>
    b.addEventListener("click", () => {
      $$("#settings-nav .settings-nav-item").forEach((x) =>
  ...
  $("#btn-attach").addEventListener("click", () => {
    ...
    input.onchange = () => {
      if (input.files[0]) ingestFile(input.files[0]);
    };
    input.click();
  });
...
function welcome() {
  /* chat starts clean — no preset message (todo: "load into a new chat page, no preset text") */
}
...
async function resolveChatKey(provider) {
  try {
    if (
      window.desktop &&
      window.desktop.keys &&
      (await window.desktop.keys.has(provider))
    ) {
...
async function sendChat() {
  ...
  try {
    const res = await fetch("/api/chat", {
      ...
      body: JSON.stringify({
        message,
        history: state.chatHistory.slice(-12, -1),
        provider,
        model,
...
async function initChatContext() {
  ...
  let cards = [];
  try {
    cards = (await api("/api/hub/cards")).cards || [];
  } catch {
    /* */
  }
  const seen = new Set();
  ...
  const builtins = [
    {
      ...
      desc: "Evaluate a product against CE/FCC/RoHS/REACH/GPSR/Prop 65 rules from its attributes.",
      cat: "commerce",
    },
    {
      name: "Amazon flat file",
      desc: "Read/write Amazon category/listing template fields; flag invalid values and recommend column fixes.",
      cat: "commerce",
    },
    {
      name: "Inventory analysis",
      desc: "Cross-check product stock vs sales signals and flag discrepancies or reorder risks.",
      cat: "commerce",
    },
  ];
...
function renderSkillsPopover() {
  ...
  pop.innerHTML = `
    ...
    ${
      state.allSkills && state.allSkills.length ?
        state.allSkills
          .map(
            (s) => `
      <button class="skill-row ${on.has(s.name) ? "on" : ""}" data-skill="${esc(s.name)}">
        ...
        <span class="skill-body"><span class="skill-name">${esc(s.name)}</span><span class="skill-desc">${esc(s.desc)}</span></span>
          ...
          .join("")
      : '<div class="skills-empty">No skills loaded — add tools in the Tool Hub.</div>'
    }
...
function pickChatDoc() {
  ...
  input.onchange = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        toast(`Doc upload failed: ${data.detail || res.statusText}`, "err");
        return;
      }
      ...
      toast(`Doc upload failed: ${e.message}`, "err");
    }
  };
  input.click();
}
...
function renderChatChips() {
  ...
  row.innerHTML = [
    ...skills.map(
      (s) =>
        `<span class="ctx-chip ctx-skill" data-kind="skill" data-name="${esc(s.name)}"><span class="ctx-ic codicon codicon-zap"></span>${esc(s.name)} <button class="ctx-x" aria-label="Remove skill">&times;</button></span>`,
    ),
    ...docs.map(
      (d) =>
        `<span class="ctx-chip ctx-doc" data-kind="doc" data-name="${esc(d.ref_id)}"><span class="ctx-ic codicon codicon-file-text"></span>${esc(d.filename)} <button class="ctx-x" aria-label="Remove document">&times;</button></span>`,
    ),
  ].join("");
  row.querySelectorAll(".ctx-chip").forEach((chip) => {
    ...
    x.addEventListener("click", () => {
      if (chip.dataset.kind === "skill") {
        state.chatSkills = state.chatSkills.filter(
          (s) => s.name !== chip.dataset.name,
        );
        renderSkillsPopover();
      } else {
        state.chatDocs = state.chatDocs.filter(
          (d) => d.ref_id !== chip.dataset.name,
        );
      }
      renderChatChips();
    });
...
document.addEventListener("click", (e) => {
  ...
  if (
    ...
    e.target.id !== "btn-skills" &&
    !(e.target.closest && e.target.closest("#btn-skills"))
  ) {
    pop.hidden = true;
    const b = $("#btn-skills");
...
async function renderIngest() {
  ...
  const fileRows =
    files.length ?
      ...
      <tr>
        ...
        <td><span class="reg-status st-${esc(
          f.status === "done" ? "pass"
          : f.status === "error" ? "fail"
          : "review",
        ...
        <td>${
          ...
          : '<span style="color:var(--muted-fg);font-size:0.6875rem">—</span>'
        }</td>
      </tr>`,
        )
        .join("")
  ...
  if (doneFiles.length) {
    ...
    try {
      ai = await api(`/api/ingest/${latest.upload_id}/ai`);
    } catch {
      /* */
    }
    const rows =
  ...
  root.querySelectorAll("[data-ai]").forEach((b) => {
    b.addEventListener("click", async () => {
      ...
      b.textContent = "…";
      try {
        const res = await api(`/api/ingest/${uploadId}/ai-process`, {
          method: "POST",
        });
...
async function renderDashboard() {
  ...
  let st = {};
  try {
    st = await api("/api/automation/stats");
  } catch (e) {
    /* */
  }
  state.stats = st;
          ...
          <div class="data-table-wrap">
            ${
              ...
              ${st.recent_ai
                ...
                <td>${r.status === "done" ? "✓" : "⚠"}</td><td>${esc(timeAgo(r.created_at))}</td></tr>`,
                )
                .join("")}
            </table>`
          ...
          <div class="data-table-wrap">
            ${
              ...
              ${st.recent_events
                .map(
                  (r) => `<tr>
                <td>${esc(r.source)}</td><td class="mono">${esc(r.type || "—")}</td><td>${esc(timeAgo(r.created_at))}</td></tr>`,
                )
                .join("")}
            </table>`
              : '<div class="empty-state">No inbound events yet — POST JSON to /webhooks/automation/&lt;source&gt;.</div>'
            }
          </div>
        </div>
      </div>
  ...
  root.querySelectorAll(".home-quick-btn").forEach((b) =>
    b.addEventListener("click", () => {
...
function openProcessModal(p = null) {
  ...
  $("#btn-save-process").addEventListener("click", async () => {
    ...
    try {
      const saved =
        p ?
          await api(`/api/processes/${p.id}`, { method: "PATCH", body })
        : await api("/api/processes", { method: "POST", body });
      closeModal();
      toast(
        `"${saved.name}" → score ${saved.automation_score}/100. ${saved.recommendation}`,
        "ok",
      );
      await refreshCounts();
...
function renderProcessDetail(p) {
  ...
  root
    .querySelector("#btn-back-procs")
    .addEventListener("click", renderProcesses);
  root.querySelectorAll(".seg-btn[data-status]").forEach((b) =>
    b.addEventListener("click", async () => {
      try {
  ...
  root
    .querySelector("#btn-edit-proc")
    .addEventListener("click", () => openProcessModal(p));
...
async function renderProcesses() {
  ...
  root
    .querySelector("#btn-new-proc")
    .addEventListener("click", () => openProcessModal());

  let procs = [];
  try {
    procs = await api("/api/processes");
  } catch (e) {
    toast(e.message, "err");
  }
  ...
  list.innerHTML = `<table class="data-table">
    ...
    ${procs
      .map(
        (p) => `<tr class="row-click" data-id="${p.id}">
      ...
      <td class="muted">${esc((p.recommendation || "").slice(0, 90))}${(p.recommendation || "").length > 90 ? "…" : ""}</td></tr>`,
      )
      .join("")}
  </table>`;
  list.querySelectorAll(".row-click").forEach((r) =>
...
function actionSummary(a) {
  return (a.actions || [])
    .map((x) => {
      const t = x.type;
      if (t === "asana_create_task")
        return `📋 Asana task → ${esc(x.target || "?")}`;
      if (t === "ai_run")
        return `✨ AI: ${esc((x.payload || {}).workflow || "?")}`;
        ...
        [
          {
            ...
            target: "Catalog Ops",
            payload: { name: "Catalog: {task_name} ready", notes: "…" },
          },
        ],
        null,
        2,
...
function openAutomationModal(a = null) {
  openModal(
    a ? "Edit automation" : "New automation",
    automationModalHTML(a),
  ...
  $("#btn-save-auto").addEventListener("click", async () => {
    ...
    try {
      ...
      if (!Array.isArray(conds) || !Array.isArray(acts))
        throw new Error("conditions and actions must be JSON arrays");
    } catch (err) {
      return toast("Invalid JSON: " + err.message, "err");
    }
    const body = {
      name: $("#f-a-name").value,
    ...
    try {
      await api(a ? `/api/automations/${a.id}` : "/api/automations", {
        method: a ? "PATCH" : "POST",
        body,
      });
      closeModal();
...
async function runAutomationNow(a) {
  ...
  if (runPanel)
    runPanel.innerHTML = '<div class="folder-loading">Running…</div>';
  ...
  try {
    ...
    try {
      ...
      return toast("Sample payload is not valid JSON", "err");
    }
    const res = await api(`/api/automations/${a.id}/run`, {
      method: "POST",
    ...
    panel.innerHTML = `
      ...
      <div class="run-steps">
        ${res.results
          ...
          <span class="step-detail">${esc(r.detail)}</span></div>`,
          )
          .join("")}
      </div>`;
    await refreshCounts();
    ...
    const panel = root.querySelector("#run-panel");
    if (panel)
      panel.innerHTML = `<div class="settings-note">⚠ ${esc(e.message)}</div>`;
    toast(e.message, "err");
  }
}
...
async function renderAutomations() {
  ...
  root
    .querySelector("#btn-new-auto")
    .addEventListener("click", () => openAutomationModal());

  let autos = [];
  try {
    autos = await api("/api/automations");
  } catch (e) {
    toast(e.message, "err");
  }
  ...
  grid.querySelectorAll(".switch input").forEach((c) =>
    c.addEventListener("change", async (e) => {
      try {
        await api(`/api/automations/${e.target.dataset.id}`, {
          method: "PATCH",
  ...
  grid.querySelectorAll(".run-payload").forEach((t) =>
    t.addEventListener("keydown", (e) => {
      if ((e.ctrlKey || e.metaKey) && e.key === "Enter") {
        const a = autos.find(
          (x) => x.id === Number(t.parentElement.dataset.id),
        );
...
async function renderAi() {
  ...
  let wfs = [];
  try {
    wfs = await api("/api/ai/workflows");
  } catch (e) {
    toast(e.message, "err");
  }
  state.aiWorkflows = wfs;
  root.querySelector("#wf-grid").innerHTML = wfs
    .map(
      (w) => `
  ...
  root
    .querySelectorAll(".btn-wf-run")
    .forEach((b) =>
      b.addEventListener("click", () => openAiModal(b.dataset.wf)),
    );

  let runs = [];
  try {
    runs = await api("/api/ai/runs?limit=50");
  } catch {
    /* */
  }
  ...
  hist.innerHTML =
    ...
    ${runs
      ...
      <td><button class="btn-secondary btn-sm btn-ai-view" data-id="${r.id}">View</button></td></tr>`,
      )
      .join("")}
  </table>`
    : '<div class="empty-state">No AI runs yet.</div>';

  root.querySelectorAll(".btn-ai-view").forEach((b) =>
    b.addEventListener("click", async () => {
      try {
...
function openAiModal(wfId) {
  ...
  $("#btn-ai-run").addEventListener("click", async () => {
    ...
    try {
      const res = await api("/api/ai/run", {
        method: "POST",
        body: { workflow: wfId, input },
      });
      resBox.innerHTML = `
...
function openSopModal(s = null) {
  ...
  $("#btn-save-sop").addEventListener("click", async () => {
    const body = {
      ...
      category: $("#f-s-cat").value,
      body: $("#f-s-body").value,
    };
    if (!body.title) return toast("Title is required", "warn");
    try {
      await api(s ? `/api/sops/${s.id}` : "/api/sops", {
        method: s ? "PATCH" : "POST",
        body,
      });
      closeModal();
...
function renderSopDoc(s) {
  ...
  root
    .querySelector("#btn-edit-sop")
    .addEventListener("click", () => openSopModal(s));
  root.querySelector("#btn-del-sop").addEventListener("click", async () => {
    if (!confirm(`Delete "${s.title}"?`)) return;
    try {
      await api(`/api/sops/${s.id}`, { method: "DELETE" });
      toast("Deleted", "ok");
      renderSops();
      refreshCounts();
    } catch (e) {
...
async function renderSops() {
  ...
  root
    .querySelector("#btn-new-sop")
    .addEventListener("click", () => openSopModal());
  root.querySelector("#sop-search").addEventListener("keydown", async (e) => {
    if (e.key === "Enter") {
      state.sopSearch = e.target.value.trim();
      await renderSops();
    }
  });
  root.querySelectorAll("#sop-cats .seg-btn").forEach((b) =>
    b.addEventListener("click", async () => {
      state.sopCat = b.dataset.cat;
      $$("#sop-cats .seg-btn").forEach((x) =>
  ...
  try {
    if (q) sops = await api("/api/sops/search?q=" + encodeURIComponent(q));
      ...
      sops = await api(
        "/api/sops" +
          (state.sopCat && state.sopCat !== "all" ?
            "?category=" + state.sopCat
          : ""),
      );
  } catch (e) {
    toast(e.message, "err");
  ...
  list.innerHTML = `<table class="data-table">
    ...
    ${sops
      ...
      <td><button class="btn-secondary btn-sm btn-sop-view" data-id="${s.id}">Open</button></td></tr>`,
      )
      .join("")}
  </table>`;
  list.querySelectorAll(".row-click, .btn-sop-view").forEach((r) =>
    r.addEventListener("click", () => {
...
async function renderIntegrations() {
  ...
  root.innerHTML = `<div class="view"><div class="view-header"><div><div class="view-title">Integrations</div>
    <div class="view-sub">Connectors that keep data in sync in real time across the stack. <b>Simulated</b> steps run when a connector has no live credentials.</div></div>
    <div class="view-actions"><button class="btn-secondary" id="btn-open-supabase"><span class="codicon codicon-database"></span> Supabase</button><button class="btn-primary" id="btn-add-integration"><span class="codicon codicon-add"></span> Add New</button></div></div>
    <div class="settings-note" id="integration-add-note" hidden>Choose a connector below, enter its credentials, then select Save. Supabase has its own secure sync workspace.</div>
    <div class="automation-grid" id="int-grid"><div class="folder-loading">Loading…</div></div></div>`;

  let ints = [];
  try {
    ints = await api("/api/integrations");
  } catch (e) {
    toast(e.message, "err");
  }
  ...
  grid.innerHTML = ints
      ...
      ${
        ...
        <label class="field" style="margin-top:0.5rem"><span>${esc(f.label)}</span>
          <input class="int-field" data-key="${esc(f.key)}" type="${f.type === "secret" ? "password" : "text"}"
            value="${esc(it.config[f.key] || "")}" placeholder="${esc(f.placeholder || "")}" autocomplete="off" /></label>`,
            )
            .join("")
        : ""
      }
    ...
    .join("");
  root
    .querySelector("#btn-open-supabase")
    .addEventListener("click", () => showView("mcpsync"));
  root.querySelector("#btn-add-integration").addEventListener("click", () => {
    const note = root.querySelector("#integration-add-note");
    note.hidden = !note.hidden;
    if (!note.hidden) grid.querySelector(".int-field")?.focus();
  });

  grid.querySelectorAll(".btn-int-save").forEach((b) =>
    b.addEventListener("click", async () => {
      ...
      try {
        const res = await api(`/api/integrations/${key}`, {
          method: "POST",
          body: { config },
        });
  ...
  grid.querySelectorAll(".btn-int-test").forEach((b) =>
    b.addEventListener("click", async () => {
      const key = b.dataset.key;
      const note = grid.querySelector(`.int-result[data-key="${key}"]`);
      note.textContent = "Testing…";
      try {
        const res = await api(`/api/integrations/${key}/test`, {
          method: "POST",
        });
...
async function renderAsana() {
  ...
  const setAsanaTab = (tab) => {
    root.querySelector("#asana-team-kpi-panel").style.display =
      tab === "team" ? "block" : "none";
    root.querySelector("#asana-perf-panel").style.display =
      tab === "perf" ? "block" : "none";
    root.querySelector("#asana-tasks-panel").style.display =
      tab === "tasks" ? "block" : "none";
    root.querySelector("#btn-tab-team-kpis").className =
      tab === "team" ? "btn-primary" : "btn-secondary";
    root.querySelector("#btn-tab-perf").className =
      tab === "perf" ? "btn-primary" : "btn-secondary";
    root.querySelector("#btn-tab-tasks").className =
      tab === "tasks" ? "btn-primary" : "btn-secondary";
  };
  root
    .querySelector("#btn-tab-team-kpis")
    .addEventListener("click", () => setAsanaTab("team"));
  root
    .querySelector("#btn-tab-perf")
    .addEventListener("click", () => setAsanaTab("perf"));
  root
    .querySelector("#btn-tab-tasks")
    .addEventListener("click", () => setAsanaTab("tasks"));
  ...
  const loadTeamKpis = async () => {
    ...
    try {
      ...
      const data = await api("/api/asana/kpis/pivot", {
        method: "POST",
        body: {
          metric,
          row_dimension: "team",
          column_dimension: grain,
          period_grain: grain,
        },
      });
      const cols = data.column_keys || [];
      const rows = data.row_keys || [];
      const byCell = new Map(
        (data.cells || []).map((c) => [`${c.row}|${c.column}`, c]),
      );
      const fmt = (c) => {
      ...
      teamTable.innerHTML = `<div class="settings-note">Team-first membership attribution · ${esc(data.metric.label)} · click a value for exact task drilldown.</div><table class="data-table" style="margin-top:0.5rem"><thead><tr><th>Team</th>${cols.map((c) => `<th class="mono">${esc(c)}</th>`).join("")}</tr></thead><tbody>${
        rows
          ...
          .join("") ||
        '<tr><td class="empty-state" colspan="2">No synced team KPI facts for this metric and period.</td></tr>'
      }</tbody></table>`;
      teamTable.querySelectorAll(".asana-kpi-cell").forEach((button) =>
        button.addEventListener("click", async () => {
          const detail = await api("/api/asana/kpis/drilldown", {
            method: "POST",
            body: {
              metric,
              team: button.dataset.team,
              period_grain: grain,
              period_start: button.dataset.period,
              date_basis: data.date_basis,
            },
          });
          const rows = detail.records || [];
          openModal(
            `KPI Drilldown — ${button.dataset.team} / ${button.dataset.period}`,
      ...
      teamTable.innerHTML = `<div class="empty-state">Team KPI table unavailable: ${esc(e.message)}</div>`;
    }
  };
  root
    .querySelector("#btn-asana-kpi-run")
    .addEventListener("click", loadTeamKpis);
  await loadTeamKpis();

  // AI Assisted Eval Button
  root
    .querySelector("#btn-asana-perf-eval-run")
    .addEventListener("click", async () => {
      const prompt =
        root.querySelector("#asana-perf-eval-input").value.trim() ||
  ...
  root.querySelector("#btn-asana-sync").addEventListener("click", async () => {
    ...
    try {
      await api("/api/asana/sync", { method: "POST", body: { mode: "all" } });
    } catch (e) {
      toast(e.message, "err");
    }
  });
  ...
  root
    .querySelector("#btn-asana-push-sb")
    .addEventListener("click", async () => {
  ...
  try {
    ...
    if (scorecards.length > 0) {
      ...
      scEl.innerHTML =
        '<div class="empty-state">No performance scorecards calculated yet. Click "Run AI Evaluation" above to generate.</div>';
    }
  } catch (e) {
    root.querySelector("#asana-perf-scorecards").innerHTML =
  ...
  try {
    ...
    let projects = [];
    try {
      projects = await api("/api/asana/projects?include_archived=false");
    } catch {
      /* */
    }
    root.querySelector("#asana-projects").innerHTML =
      projects.length ?
        `<table class="data-table"><tr><th>Name</th><th>Team</th></tr>
         ${projects
           ...
           .map(
             (p) =>
               `<tr><td>${esc(p.name)}</td><td>${esc(p.team_name || "—")}</td></tr>`,
           )
           .join("")}</table>`
    ...
    let tasks = [];
    try {
      tasks = await api("/api/asana/tasks?limit=30");
    } catch {
      /* */
    }
    root.querySelector("#asana-tasks").innerHTML =
      tasks.length ?
...
async function renderEvents() {
  ...
  let evs = [];
  try {
    evs = await api("/api/events?limit=200");
  } catch (e) {
    toast(e.message, "err");
  }
  ...
  list.innerHTML =
    ...
    ${evs
      .map(
        (
          e,
        ) => `<tr><td>${esc(e.source)}</td><td class="mono">${esc(e.type || "—")}</td>
      <td class="mono muted">${esc(String(e.payload || "").slice(0, 120))}</td><td>${esc(timeAgo(e.created_at))}</td></tr>`,
      )
      .join("")}
  </table>`
...
async function renderRequests() {
  ...
  root
    .querySelector("#btn-req-refresh")
    .addEventListener("click", renderRequests);
  let reqs = [];
  try {
    reqs = await api("/api/requests?limit=200");
  } catch (e) {
    toast(e.message, "err");
  }
  ...
  list.innerHTML =
    ...
    ${reqs
      .map(
        (
          r,
        ) => `<tr><td><span class="method-chip m-${esc(r.method)}">${esc(r.method)}</span></td>
      <td class="mono">${esc(r.path)}</td><td>${r.status}</td>
      <td>${Math.round(r.latency_ms)}ms</td><td>${esc(timeAgo(r.created_at))}</td></tr>`,
      )
      .join("")}
  </table>`
...
async function openFolder(path) {
  ...
  try {
    ...
    if (!res.dirs.length && !res.files.length)
      tree.innerHTML = '<div class="folder-empty">Empty folder</div>';
  } catch (e) {
    $("#folder-tree").innerHTML =
      `<div class="folder-error">${esc(e.message)}</div>`;
  }
}
  ...
  try {
    ...
    (jobs || []).forEach((j) => {
      ...
      items.push({
        ...
        active,
        progress: j.progress,
        text: j.message || meta.title,
      });
    });
    (st.recent_ai || []).forEach((r) =>
      items.push({
        t: r.created_at,
        icon: "chat-sparkle",
        title: `AI ${r.workflow}`,
        ...
        progress: null,
        text:
          `${r.provider || ""}${r.model ? " · " + r.model : ""}`.trim() ||
          r.workflow,
      }),
    );
    evs.forEach((e) =>
...
async function renderSettingsTab(tab) {
  ...
  if (tab === "integrations") {
    box.innerHTML = `<div class="settings-pane active"><div class="settings-section">
      <div class="settings-title"><span class="codicon codicon-plug"></span> Integrations</div>
      <div class="settings-note">Manage connector credentials and open the dedicated Supabase sync workspace.</div>
      <div class="settings-actions"><button class="btn-primary" id="settings-open-integrations"><span class="codicon codicon-plug"></span> Open Integrations</button><button class="btn-secondary" id="settings-open-supabase"><span class="codicon codicon-database"></span> Open Supabase Sync</button></div>
    </div></div>`;
    box
      .querySelector("#settings-open-integrations")
      .addEventListener("click", () => {
        $("#settings-backdrop").hidden = true;
        showView("integrations");
      });
    box
      .querySelector("#settings-open-supabase")
      .addEventListener("click", () => {
        $("#settings-backdrop").hidden = true;
        showView("mcpsync");
      });
  ...
  if (tab === "spine") {
    box.innerHTML =
      '<div class="settings-pane active"><div class="folder-loading">Loading Spine registry…</div></div>';
    try {
      const snapshot = await api("/api/spine/snapshot");
      const sections = [
        [
          "Registry",
          snapshot.registry,
          ["label", "kind", "route", "status_key"],
        ],
        ["Statuses", snapshot.statuses, ["label", "status_key", "category"]],
        [
          "Lifecycles",
          snapshot.lifecycles,
          ["label", "lifecycle_key", "terminal"],
        ],
        ["File Types", snapshot.file_types, ["label", "extension", "category"]],
        ["Models", snapshot.models, ["label", "provider_id", "model_id"]],
        [
          "Model Presets",
          snapshot.model_presets,
          ["label", "preset_key", "model_id"],
        ],
        ["Workflow Nodes", snapshot.nodes, ["label", "node_type", "category"]],
        [
          "Datasets",
          snapshot.datasets,
          ["label", "dataset_key", "entity_type"],
        ],
        [
          "Global Filters",
          snapshot.filters,
          ["label", "filter_key", "entity_type"],
        ],
      ];
      const table = (title, rows, columns) =>
        `<div class="settings-section"><div class="settings-title">${esc(title)} <span class="pill-int pill-int-configured">${rows?.length || 0}</span></div><div class="data-table-wrap"><table class="data-table"><thead><tr>${columns.map((c) => `<th>${esc(c.replaceAll("_", " "))}</th>`).join("")}</tr></thead><tbody>${
          (rows || [])
            .slice(0, 100)
            .map(
              (row) =>
                `<tr>${columns.map((c) => `<td>${esc(row[c] == null ? "—" : String(row[c]))}</td>`).join("")}</tr>`,
            )
            .join("") ||
          `<tr><td colspan="${columns.length}">No entries</td></tr>`
        }</tbody></table></div></div>`;
      box.innerHTML = `<div class="settings-pane active"><div class="settings-section"><div class="settings-title"><span class="codicon codicon-symbol-namespace"></span> Conductor Spine</div><div class="settings-note">Local-first canonical metadata shared by features, models, workflow nodes, datasets, lifecycle definitions, file types, and global filters.</div></div>${sections.map(([title, rows, columns]) => table(title, rows, columns)).join("")}</div>`;
    } catch (e) {
      box.innerHTML = `<div class="settings-pane active"><div class="empty-state">Spine unavailable: ${esc(e.message)}</div></div>`;
    }
    return;
  }
  ...
  if (tab === "chat") {
    ...
    const provs =
      provsRes.status === "fulfilled" ? provsRes.value : { providers: [] };
    const disc =
      discRes.status === "fulfilled" ? discRes.value : { models: [] };
    const keys = keysRes.status === "fulfilled" ? keysRes.value : { keys: {} };

    const providerCards = (provs.providers || [])
      .map((p) => {
        ...
        const healthBadge =
          p.configured ?
            health.healthy ?
              '<span class="pill-int pill-int-configured">healthy</span>'
            : `<span class="pill-int pill-int-missing" title="${esc(health.error || "")}">unhealthy</span>`
          : "";
        const modelOpts = (p.models || [])
          .map(
            (m) =>
              `<option value="${esc(m.id)}" ${p.defaultModelId === m.id ? "selected" : ""}>${esc(m.id)}</option>`,
          )
          .join("");
    ...
    const chatModels = (provs.providers || [])
      .filter((p) => p.configured)
      .map((p) =>
        (p.models || [])
          .map(
            (m) =>
              `<option value="${esc(m.id)}" data-provider="${esc(p.id)}">${esc(p.label)} — ${esc(m.id)}</option>`,
          )
          .join(""),
      )
      .join("");
    ...
    const ggufRow = (m) =>
      `<option value="${esc(m.id)}" ${cfg.llama_model === m.id ? "selected" : ""}>${esc(m.name)} — ${(m.sizeBytes / 1073741824).toFixed(1)} GB (${esc(srcLabel(m.sourceDir))})</option>`;
    const discoveryNote =
      discovered.length ?
            ...
            <label class="field"><span>Provider</span>
              <select id="s-provider">
                ${(provs.providers || [])
                  ...
                  .map(
                    (p) =>
                      `<option value="${esc(p.id)}" ${(cfg.provider || "deepseek") === p.id ? "selected" : ""}>${esc(p.label)}</option>`,
                  )
                  .join("")}
    ...
    const saveProviderKey = async (pid) => {
      ...
      await api("/api/chat/keys", {
        method: "POST",
        body: { providerId: pid, value, encrypted },
      });
      toast(
        `${pid} key saved${encrypted ? " (encrypted via OS keychain)" : ""}`,
        "ok",
      );
      renderSettingsTab("chat");
    };
    box
      .querySelectorAll(".prov-save")
      .forEach((b) =>
        b.addEventListener("click", () => saveProviderKey(b.dataset.pid)),
      );
    box.querySelectorAll(".prov-remove").forEach((b) =>
      b.addEventListener("click", async () => {
    ...
    box.querySelectorAll(".prov-enabled").forEach((cb) =>
      cb.addEventListener("change", async () => {
        await api("/api/chat/config", {
          method: "POST",
          body: {
            provider: cfg.provider || "deepseek",
            providers: { [cb.dataset.pid]: { enabled: cb.checked } },
          },
        });
    ...
    box
      .querySelector("#btn-llama-start")
      .addEventListener("click", async () => {
        try {
          const model = box.querySelector("#s-llama-model").value;
          if (!model)
            return toast("Pick a discovered GGUF model first", "warn");
          const res = await api("/api/llama/start", {
            method: "POST",
            body: {
              model,
              ctx: Number(box.querySelector("#s-llama-ctx").value || 4096),
            },
          });
          toast(
            `Llama server up on port ${res.port}${res.reused ? " (adopted existing server)" : ""}`,
            "ok",
          );
          box.querySelector("#s-llama-status").textContent =
            `Local server: running · ${res.model}`;
        } catch (e) {
          toast(e.message, "err");
        }
      });
    box.querySelector("#btn-llama-stop").addEventListener("click", async () => {
      try {
        await api("/api/llama/stop", { method: "POST" });
        toast("Llama server stopped", "ok");
      } catch (e) {
        toast(e.message, "err");
      }
    });
    box.querySelector("#btn-save-chat").addEventListener("click", async () => {
      ...
      try {
        ...
        toast(
          `Saved — provider ${res.provider}${res.configured ? " (key set)" : " (no key yet)"}`,
          "ok",
        );
    ...
    $("#btn-test-chat").addEventListener("click", async () => {
      try {
        ...
        toast(
          res.ok ? "Chat test passed" : (
            `Chat test failed: ${text.slice(0, 140)}`
          ),
          res.ok ? "ok" : "err",
        );
      } catch (e) {
  ...
  if (tab === "asana") {
    ...
    box.querySelector("#btn-save-asana").addEventListener("click", async () => {
      ...
      try {
        await api("/api/asana/config", { method: "POST", body });
        toast("Asana config saved", "ok");
      } catch (e) {
        toast(e.message, "err");
      }
    });
  ...
  if (tab === "spapi") {
    let st = {};
    try {
      st = await api("/api/productpipeline/status");
    } catch {
      /* */
    }
  ...
  if (tab === "about") {
    ...
    if (window.desktop && window.desktop.getUpdateInfo) {
      try {
        updInfo = await window.desktop.getUpdateInfo();
      } catch {
        /* */
      }
    }
    ...
    const setStatus = (msg) => {
      const el = box.querySelector("#upd-status");
      if (el) el.textContent = msg;
    };
    box
      .querySelector("#btn-check-updates")
      .addEventListener("click", async () => {
        if (!window.desktop || !window.desktop.checkForUpdates) {
          setStatus(
            "Updates are only available in the installed (packaged) app.",
          );
          return;
        }
        ...
        if (!info || !info.isPackaged) {
          setStatus(
            `Running the dev build (v${info ? info.version : "—"}) — updates apply only to the installed app.`,
          );
          return;
        }
        ...
        try {
          ...
          else if (res.reason === "updates-disabled")
            setStatus("Updates are not enabled for this build.");
          else if (res.downloaded) {
            state.updateReady = { ready: true, version: res.downloaded };
            const b = box.querySelector("#btn-install-update");
            if (b) b.hidden = false;
            setStatus(
              `Conductor ${res.downloaded} downloaded and ready — restart to install.`,
            );
          } else if (res.available)
            setStatus(`Update v${res.version} available — downloading…`);
    ...
    box.querySelector("#btn-install-update").addEventListener("click", () => {
      if (window.desktop && window.desktop.installUpdate)
...
Tone: Analytical, precise, metric-driven. Always cite relevant product SKUs, ASINs, and task GIDs.`;
  ...
  $("#btn-tmpl-default").addEventListener("click", () => {
    textarea.value = defaultPrompt;
  });
  $("#btn-tmpl-franky").addEventListener("click", () => {
    textarea.value = frankyPrompt;
  });
  $("#btn-tmpl-ops").addEventListener("click", () => {
    textarea.value = opsPrompt;
  });
...
async function refreshStatusbar() {
  try {
    ...
    $("#status-tokens").textContent =
      `${fmtNum((tu.total_tokens || 0) + (tu.input_tokens || 0) + (tu.output_tokens || 0))} tok`;
...
async function refreshCounts() {
  const set = (key, val) => {
    ...
    document
      .querySelectorAll(`.sidebar-count[data-count="${key}"]`)
      .forEach((el) => {
...
function wireUpdates() {
  ...
  const markReady = (version) => {
    ...
    const st = $("#upd-status");
    if (st)
      st.textContent = `Conductor ${version} downloaded and ready — restart to install.`;
  };
  window.desktop.onUpdateEvent((ev) => {
    ...
    if (ev.event === "update-available") {
      ...
      const st = $("#upd-status");
      if (st)
        st.textContent = `Downloading Conductor ${downloadingVersion || ""}… ${pct}%`;
...
function warmLoad() {
  return (window.ConductorData || { preload: () => Promise.resolve() }).preload(
    ["products", "checks", "people"],
  );
}
...
async function boot() {
  ...
  try {
    restoredView = localStorage.getItem("conductor.view");
  } catch {
    /* storage unavailable */
  }
  if (restoredView && restoredView !== "chat" && VIEW_RENDERERS[restoredView]) {
    try {
      ...
      console.error(
        `[boot] failed to restore view "${restoredView}":`,
        err?.message || err,
      );
      showView("chat");
    }
...
function vvLoadSettings() {
  const pts =
    localStorage.getItem("conductor.vv.pts") ||
    "CONDITIONER\nSHAMPOO\nHAIR_CLEANER_CONDITIONER\nHAIR_STYLING_AGENT\nSKIN_MOISTURIZER\nSKIN_CLEANING_AGENT\nGROCERY\nBEAUTY\nSHIRT";
  const themes =
    localStorage.getItem("conductor.vv.themes") ||
    "Size\nColor\nSizeColor\nFlavor\nScent\nCount\nStyleName";
  vvState.settings.pts = pts;
  vvState.settings.themes = themes;
  vvState.settings.grocery =
    localStorage.getItem("conductor.vv.grocery") !== "false";
}
...
function vvSaveSettings() {
  ...
  localStorage.setItem(
    "conductor.vv.grocery",
    vvState.settings.grocery ? "true" : "false",
  );
}
...
async function renderVariation() {
  ...
  root.querySelector("#vv-import").addEventListener("click", () => {
    ...
    inp.onchange = () => {
      ...
      rd.onload = () => {
        ...
        if (grid.length < 2) {
          toast("CSV has no data rows", "err");
          return;
        }
        const header = grid[0];
        const objs = grid.slice(1).map((cells) => {
          ...
          header.forEach((h, idx) => {
            o[String(h || "").trim()] = cells[idx] ?? "";
          });
          return o;
        });
        let added = 0;
        objs.forEach((o) => {
          const parent = vvGetVal(o, [
            "target_parent",
            "current_parent",
            "parent",
          ]);
  ...
  root.querySelector("#vv-export").addEventListener("click", () => {
    ...
    vvState.rows.forEach((r) =>
      lines.push(
        [r.parent, r.asin, r.brand, r.category, r.pt, r.theme, r.form, r.count]
  ...
  bind("#vv-pts", () => {
    vvState.settings.pts = root.querySelector("#vv-pts").value;
    vvSaveSettings();
  });
  bind("#vv-themes", () => {
    vvState.settings.themes = root.querySelector("#vv-themes").value;
    vvSaveSettings();
  });
  bind("#vv-grocery", () => {
    vvState.settings.grocery = root.querySelector("#vv-grocery").checked;
    vvSaveSettings();
    vvValidate(root);
  });
  root
    .querySelectorAll(".vv-row input")
    .forEach((inp) => inp.addEventListener("input", () => vvValidate(root)));
  root.querySelectorAll(".vv-x").forEach((b) =>
    b.addEventListener("click", () => {
      vvState.rows.splice(Number(b.dataset.del), 1);
      renderVariation();
    }),
  );
  vvValidate(root);
...
function vvValidate(root) {
  ...
  rowEls.forEach((tr) => {
    ...
    tr.querySelectorAll("input").forEach((inp) =>
      inp.closest("td").classList.remove("vv-cell-error", "vv-cell-warn"),
    );
    if (!r.parent) {
      tr.querySelector(".vv-pill").className = "vv-pill vv-empty";
      tr.querySelector(".vv-pill").textContent = "--";
      return;
    }
  ...
  for (const [parent, fam] of Object.entries(families)) {
    ...
    if (fam.length <= 1) {
      status = "fail";
      msgs.push("Variation families need more than 1 child ASIN.");
    }
    for (let k = 1; k < fam.length; k++) {
      const c = fam[k].r;
      const flagCell = (f) => {
        const td = fam[k].tr.querySelector(`[data-f="${f}"]`).closest("td");
        td.classList.add("vv-cell-error");
      };
      if (c.brand !== base.brand) {
        status = "fail";
        msgs.push("Brand mismatch across children.");
        flagCell("brand");
      }
      if (c.category !== base.category) {
        status = "fail";
        msgs.push("Category mismatch across children.");
        flagCell("category");
      }
      if (c.pt !== base.pt) {
        status = "fail";
        msgs.push("Product Type mismatch across children.");
        flagCell("pt");
      }
      if (c.theme !== base.theme) {
        status = "fail";
        msgs.push("Theme mismatch across children.");
        flagCell("theme");
      }
      if (
        vvState.settings.grocery &&
        ...
        msgs.push("Item Form varies within family.");
      }
    }
    fam.forEach(({ tr }) => {
      const pill = tr.querySelector(".vv-pill");
      pill.className = `vv-pill vv-${
        status === "pass" ? "pass"
        : status === "fail" ? "fail"
        : "warn"
      }`;
    ...
    cards.push(
      `<div class="vv-card vv-${status}"><div class="vv-card-head"><span class="vv-pill vv-${status}">${status.toUpperCase()}</span> <span class="vv-parent mono">${esc(parent)}</span> <span class="vv-n">${fam.length} children</span></div><div class="vv-card-msgs">${msgs.map((m) => `<div>· ${esc(m)}</div>`).join("") || "<div>All variation attributes consistent.</div>"}</div></div>`,
    );
  }
  report.innerHTML = `
...
async function renderReports() {
  ...
  root.querySelector("#rpt-generate").addEventListener("click", async () => {
    ...
    try {
      ...
      btn.disabled = false;
      btn.textContent = "Generate CDQ Analysis";
    }
  });
  root.querySelectorAll("[data-view-report]").forEach((b) =>
    b.addEventListener("click", async () => {
      try {
  ...
  root.querySelectorAll("[data-del-report]").forEach((b) =>
    b.addEventListener("click", async () => {
      try {
...
function renderCdqReport(report, root) {
  ...
  const k = d.kpis || {};
  const gradeColors = {
    A: "var(--t-function-success, #30A46C)",
    B: "#7dd3c0",
    C: "var(--yellow, #F5A524)",
    D: "#ff6b6b",
    U: "var(--red, #E5484D)",
  ...
  const grades = (d.grades || [])
    .map(
      ...
      <div class="cdq-grade-row"><span class="cdq-grade-label mono">${esc(g.grade)}</span>
        ...
        <span class="cdq-grade-n mono">${fmtNum(g.count)}</span></div>`,
    )
    .join("");
  const comps = (d.components || [])
    .map(
      ...
      <div class="cdq-comp-row"><span class="cdq-comp-label">${esc(c.name)} <small>(${c.weight}%)</small></span>
        <div class="cdq-bar"><div class="cdq-bar-fill" style="width:${Math.min(100, c.score).toFixed(1)}%;background:${
          c.score >= 85 ? "var(--t-function-success, #30A46C)"
          : c.score >= 70 ? "var(--t-function-primary, #0053fd)"
          : "var(--yellow, #F5A524)"
        }"></div></div>
        <span class="cdq-comp-n mono">${c.score}%</span></div>`,
    )
    .join("");
  const plan = (d.action_plan || [])
    .map(
      ...
      <tr><td><span class="badge-grade b-${esc(p.priority.toLowerCase())}">${esc(p.priority)}</span></td><td>${esc(p.issue)}</td>
        <td class="mono">${fmtNum(p.asins)}</td><td class="mono">${p.pct}%</td><td style="font-size:0.75rem">${esc(p.action)}</td></tr>`,
    )
    .join("");
  const fixes = (d.top_fixes || [])
    .map(
      (f) => `
      <tr><td class="mono">${f.rank}</td><td class="mono">${esc(f.sku)}</td><td>${esc(f.brand)}</td>
        <td><span class="badge-grade b-${esc(f.grade.toLowerCase())}">${esc(f.grade)}</span></td><td style="font-size:0.75rem">${esc(f.issue)}</td></tr>`,
    )
    .join("");
  const findings = (d.findings || [])
    .map(
      (f) => `
      <div class="insight-box ${esc(f.tone)}"><strong>${esc(f.title)}</strong> — ${esc(f.body)}</div>`,
    )
    .join("");
...
async function renderGuidelines() {
  ...
  try {
    const [g, a, o] = await Promise.all([
      api("/api/guidelines"),
      api("/api/guidelines/attributes"),
      api("/api/guidelines/options"),
    ]);
    guidelinesCache = {
      list: g.guidelines || [],
      attrs: a.attributes || [],
  ...
  const byAttr = {};
  list.forEach((g) => {
    (byAttr[g.attribute] = byAttr[g.attribute] || []).push(g);
  });
  const groups = Object.entries(byAttr)
    .map(
      ([attr, gs]) => `
      <div class="guide-group">
        ...
        ${gs
          ...
          <div class="guide-row">
            ...
            <span class="chip-kind k-${
              g.severity === "blocker" ? "flag"
              : g.severity === "warning" ? "flag"
              : "enrichment"
            }">${esc(g.severity)}</span>
  ...
  root
    .querySelector("#g-add")
    .addEventListener("click", () => openGuidelineForm(null));
  root.querySelectorAll("[data-edit-g]").forEach((b) =>
    b.addEventListener("click", () => {
      const g = list.find((x) => x.id === Number(b.dataset.editG));
      if (g) openGuidelineForm(g);
    }),
  );
  root.querySelectorAll("[data-del-g]").forEach((b) =>
    b.addEventListener("click", async () => {
      try {
        await api(`/api/guidelines/${b.dataset.delG}`, { method: "DELETE" });
        toast("Guideline removed", "ok");
        renderGuidelines();
      } catch (e) {
        toast(`Delete failed: ${e.message}`, "err");
      }
...
function openGuidelineForm(g) {
  ...
  const ruleTypes = opts.rule_types || [
    "required",
    "allowed_values",
    "pattern",
    "min_length",
  ...
  const groupValues =
    grouping === "category" ? catVals
    : grouping === "market" ? mktVals
    : [];
  ...
  openModal(
    g ? "Edit guideline" : "Add guideline",
    body,
  ...
  $("#g-grouping").addEventListener("change", (e) => {
    const vals =
      e.target.value === "category" ? catVals
      : e.target.value === "market" ? mktVals
      : [];
    const lbl = $("#g-gv-label");
    lbl.hidden = vals.length === 0;
    if (vals.length) {
      $("#g-group-value").innerHTML =
        vals
          .map((v) => `<option value="${esc(v)}">${esc(v)}</option>`)
          .join("") + '<option value="">— any / custom —</option>';
    }
  });
  $("#g-save").addEventListener("click", async () => {
    const payload = {
      ...
      group_value:
        $("#g-group-value") && !$("#g-gv-label").hidden ?
          $("#g-group-value").value
        : "",
    ...
    if (!payload.attribute) {
      toast("Attribute is required", "err");
      return;
    }
    try {
      if (g)
        await api(`/api/guidelines/${g.id}`, { method: "PUT", body: payload });
      else await api("/api/guidelines", { method: "POST", body: payload });
      closeModal();
      toast("Guideline saved", "ok");
      renderGuidelines();
    } catch (e) {
...
function getLayout() {
  try {
    return Object.assign(
      { sidebar: true, folders: true, statusbar: true },
      JSON.parse(localStorage.getItem(layoutKey)),
    );
  } catch {
    return { sidebar: true, folders: true, statusbar: true };
  }
}
...
function renderBreadcrumb(path, root) {
  ...
  crumbs.forEach((c, i) => {
    ...
    const b = el(
      "button",
      `bc-crumb${i === crumbs.length - 1 ? " current" : ""}`,
      c.name,
    );
    b.addEventListener("click", () => {
      folderState.open = {};
      loadFolderTree(c.path);
    });
    bc.appendChild(b);
  });
}
function folderRowForDir(d) {
  ...
  const counts = [];
  if (d.notes)
    counts.push(`<span class="codicon codicon-file-text"></span> ${d.notes}`);
  if (d.files) counts.push(`${d.files}`);
  if (d.buckets)
    counts.push(`<span class="codicon codicon-briefcase"></span> ${d.buckets}`);
  row.innerHTML = `
...
function fileRow(f) {
  ...
  row.addEventListener("click", () =>
    toast(`${f.name} — ${fmtBytes(f.size)}`, "info"),
  );
  return row;
}
async function toggleFolder(row, d) {
  ...
  if (isOpen) {
    ...
    row.querySelector(".folder-ic").className =
      "folder-ic codicon codicon-folder";
  ...
  row.querySelector(".folder-chevron").textContent = "▾";
  row.querySelector(".folder-ic").className =
    "folder-ic codicon codicon-folder-opened";
  if (!childWrap || !childWrap.classList.contains("folder-children")) {
    ...
    try {
      const sub = await api(
        `/api/vault/tree?path=${encodeURIComponent(d.path)}`,
      );
...
$("#pane-search").addEventListener("input", (e) => {
  ...
  $$("#folder-tree .folder-row").forEach((r) => {
    const name = (
      r.querySelector(".folder-name")?.textContent || ""
    ).toLowerCase();
...
function resetTokens() {
  ...
  updateUiDirty();
  toast(
    "Tokens reset to the default custom palette — hit Save to persist",
    "info",
  );
}
...
async function pasteUiJson() {
  try {
    ...
    toast(`Paste failed: ${e.message}`, "err");
  }
}
function themePreviewHtml(t) {
  const bg = t.background1,
    side = t.background2,
    fg = t.colorFont.heading,
    muted = t.colorFont.muted,
    prim = t.function.primary;
...
function tokenRow(def) {
  ...
  if (def.type === "color") {
    ...
    const commit = (v) => {
      setPath(uiState.tokens, def.path, parseFloat(v));
      onTokenChange();
    };
    ...
    num.value = cur ?? "";
    num.addEventListener("input", () => {
      setPath(uiState.tokens, def.path, parseFloat(num.value));
      onTokenChange();
    });
    ctl.appendChild(num);
    ...
    cb.addEventListener("change", () => {
      setPath(uiState.tokens, def.path, cb.checked);
      onTokenChange();
    });
    ctl.appendChild(cb);
    ...
    sel.addEventListener("change", () => {
      setPath(uiState.tokens, def.path, sel.value);
      onTokenChange();
    });
    ctl.appendChild(sel);
    ...
    ta.addEventListener("change", () => {
      try {
        ...
        toast("Overrides JSON invalid — not applied", "err");
      }
    });
    row.appendChild(label);
    row.appendChild(ta);
...
function renderAppearance() {
  ...
  grid.innerHTML = "";
  const entries = [
    ["custom", "Custom", "Your design-token blob — edit below"],
    ...Object.entries(THEME_PRESETS).map(([k, v]) => [k, v.label, v.desc]),
  ];
  for (const [name, label, desc] of entries) {
    ...
    const blob =
      name === "custom" ?
        uiState.tokens
      : THEME_PRESETS[name][uiState.mode] || THEME_PRESETS[name].dark;
  ...
  for (const grp of TOKEN_SCHEMA) {
    ...
    if (
      grp.label === "Function colors" ||
      grp.label === "Gradients" ||
      grp.label === "Backgrounds"
    )
      d.open = true;
...
function renderSkins() {
  ...
  if (!list) return;
  list.innerHTML = "";
  const names = Object.keys(uiState.skins || {}).sort((a, b) =>
    a.localeCompare(b),
  );
  if (!names.length) {
    list.appendChild(
      el(
        "div",
        "skins-empty",
        'No skins saved yet — tune a theme, name it, and hit "Save current as skin".',
      ),
    );
    return;
  }
...
async function saveSkin(name) {
  ...
  if (!n) {
    toast("Give the skin a name first", "warn");
    return;
  }
  try {
    const r = await api("/api/ui/skins", {
      method: "POST",
      body: {
        name: n,
        theme: uiState.theme,
    ...
    toast(`Save skin failed: ${e.message}`, "err");
  }
}
function loadSkin(name) {
  const s = uiState.skins[name];
  if (!s) {
    toast(`Skin "${name}" not found`, "err");
    return;
  }
  uiState.theme =
    s.theme === "custom" || THEME_PRESETS[s.theme] ? s.theme : "custom";
...
async function deleteSkin(name) {
  try {
    const r = await api(`/api/ui/skins/${encodeURIComponent(name)}`, {
      method: "DELETE",
    });
...
function syncJsonEditor() {
  ...
  if (ta)
    ta.value = JSON.stringify(
      { theme: uiState.theme, mode: uiState.mode, tokens: uiState.tokens },
      null,
      2,
    );
}
function applyJsonEditor(save) {
  try {
    ...
    uiState.theme =
      v.theme === "custom" || THEME_PRESETS[v.theme] ? v.theme : "custom";
...
function renderAppearanceTab() {
  ...
  $("#btn-ui-save").addEventListener("click", async () => {
    try {
      await saveUiConfig();
      toast("Theme saved to data/ui.json", "ok");
    } catch (e) {
      toast(`Save failed: ${e.message}`, "err");
    }
  });
  ...
  $("#btn-skin-save").addEventListener("click", () =>
    saveSkin($("#skin-name").value),
  );
  $("#skin-name").addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      e.preventDefault();
      saveSkin($("#skin-name").value);
    }
...
function renderAdvancedTab() {
  ...
  syncJsonEditor();
  box
    .querySelector("#btn-ui-json-apply")
    .addEventListener("click", () => applyJsonEditor(false));
  box
    .querySelector("#btn-ui-json-save")
    .addEventListener("click", () => applyJsonEditor(true));
}
  ...
  return (
    {
      ...
      pass: "verdict-pass",
      not_applicable: "verdict-pass",
    }[status] || "verdict-pass"
  );
}
function verdictLabel(status) {
  return (
    { fail: "FAIL", review: "REVIEW", pass: "PASS", not_applicable: "N/A" }[
      status
    ] ||
    status ||
    "PASS"
...
function complianceReportHTML(product, checks) {
  const score =
    checks.length ?
      Math.round(checks.reduce((a, c) => a + (c.score || 0), 0) / checks.length)
    : 0;
  const verdict =
    score >= 80 ? "pass"
    : score >= 50 ? "review"
...
function openProductModal() {
  ...
  $("#btn-save-product").addEventListener("click", async () => {
    ...
    try {
      ...
      return toast("Attributes JSON is invalid", "err");
    }
    closeModal();
    await submitProduct({
      sku,
...
async function submitProduct(item) {
  try {
    ...
    toast(
      `"${item.sku}" checked — ${typeof score === "number" ? score + "/100" : "done"}`,
      typeof score === "number" && score < 70 ? "warn" : "ok",
    );
    showView("products");
    invalidateWarm();
...
async function submitBulk(items) {
  try {
    const res = await api("/webhooks/ingest", {
      method: "POST",
      body: { products: items },
    });
...
async function renderChecks() {
  ...
  root.querySelector("#btn-gen-compl-script").addEventListener("click", () => {
    const prompt =
      root.querySelector("#compl-script-input").value.trim() ||
      "Verify product compliance requirements";
...
`;
  ...
  let rows;
  try {
    rows = await window.ConductorData.get("checks");
  } catch (e) {
    toast(e.message, "err");
    rows = [];
  }
  ...
  body.innerHTML = `<table class="data-table">
    ...
    ${rows
      .map(
        (r) => `<tr class="row-click" data-id="${r.product.id}">
      ...
      <td><span class="score-chip ${scoreClass(r.score)}">${r.score}</span></td></tr>`,
      )
      .join("")}
  </table>`;
  body
    ...
    .forEach((tr) =>
      tr.addEventListener("click", () =>
        renderProductDetail(Number(tr.dataset.id)),
      ),
    );
}
...
async function renderProducts() {
  ...
  let prods;
  try {
    prods = await window.ConductorData.get("products");
  } catch (e) {
    toast(e.message, "err");
    prods = [];
  }
  ...
  body.innerHTML = `<table class="data-table">
    ...
    ${prods
      ...
      <td>${esc(p.source || "—")}</td><td>${esc(timeAgo(p.created_at))}</td></tr>`,
      )
      .join("")}
  </table>`;
  body
...
async function renderProductDetail(id) {
  ...
  let data;
  try {
    data = await api(`/api/products/${id}`);
  } catch (e) {
...
async function renderCatalog() {
  ...
  let files = [];
  try {
    files = await api("/api/files?limit=100");
  } catch (e) {
    toast(e.message, "err");
  }
  ...
  body.innerHTML = `<table class="data-table">
    ...
    ${files
      ...
      <td><span class="pill-int pill-int-${
        f.status === "done" ? "configured"
        : f.status === "error" ? "unconfigured"
        : "simulated"
      }">${esc(f.status)}</span></td>
      <td>${f.record_count || "—"}</td><td>${fmtBytes(f.total_size)}</td>
      <td class="muted">${esc(f.error || "")}</td><td>${esc(timeAgo(f.created_at))}</td></tr>`,
      )
      .join("")}
  </table>`;
}
...
async function renderAgents() {
  ...
  if (!agents.length) {
    grid.innerHTML = '<div class="empty-state">No agents discovered.</div>';
    return;
  }
  grid.innerHTML = agents
    ...
    .join("");
  grid
    .querySelectorAll(".btn-agent-run")
    .forEach((b) =>
      b.addEventListener("click", () =>
        runQuickAction(b.dataset.id, root.querySelector("#agent-result")),
      ),
    );
}
...
async function runQuickAction(agentId, resultEl) {
  resultEl.innerHTML =
    '<div class="folder-loading">Running quick action…</div>';
  try {
...
async function renderTasksView() {
  ...
  root
    .querySelector("#btn-tasks-import")
    .addEventListener("click", async () => {
      try {
  ...
  root.querySelector("#btn-tasks-clear").addEventListener("click", async () => {
    try {
      await api("/api/tasks/clear-done", { method: "POST" });
      toast("Cleared done tasks", "ok");
      renderTasksView();
    } catch (e) {
      toast(e.message, "err");
    }
  ...
  let tasks = [];
  try {
    tasks = await api("/api/tasks?limit=500");
  } catch (e) {
    toast(e.message, "err");
  }
  ...
  if (!tasks.length) {
    body.innerHTML =
      '<div class="empty-state">Queue is empty — import from Obsidian or let the automation engine open tasks.</div>';
    return;
  }
  body.innerHTML = `<table class="data-table">
    ...
    ${tasks
      ...
      <td>${
        ...
        : `<button class="btn-secondary btn-sm btn-task-open" data-id="${t.id}" title="Reopen">↺</button>`
      }</td>
      ...
      <td>${esc(timeAgo(t.created_at))}</td></tr>`,
      )
      .join("")}
  </table>`;
  ...
  body
    .querySelectorAll(".btn-task-done")
    .forEach((b) =>
      b.addEventListener("click", () => toggle(Number(b.dataset.id), "done")),
    );
  body
    .querySelectorAll(".btn-task-open")
    .forEach((b) =>
      b.addEventListener("click", () => toggle(Number(b.dataset.id), "open")),
    );
}
...
async function renderRegs() {
  ...
  let regs = [];
  try {
    regs = await api("/api/regulations");
  } catch (e) {
    toast(e.message, "err");
  }
  ...
  if (!regs.length) {
    body.innerHTML =
      '<div class="empty-state">No regulations catalog available.</div>';
    return;
  }
  body.innerHTML = `<table class="data-table">
...
function parseComposer(text) {
  ...
  if (t.startsWith("{") || t.startsWith("[")) {
    try {
      ...
      if (obj && typeof obj === "object" && (obj.sku || obj.name))
        return { type: "product", item: obj };
    } catch {
...
async function ingestFile(file) {
  ...
  try {
    ...
    if (file.size <= 20 * 1024 * 1024) {
      ...
      const init = await api("/api/ingest/init", {
        method: "POST",
      ...
      for (let i = 0; i < total; i++) {
        ...
        const res = await fetch(`/api/ingest/${init.upload_id}/chunk/${i}`, {
          method: "PUT",
          body: chunk,
        });
        if (!res.ok)
          throw new Error(
            (await res.json().catch(() => ({}))).detail || `chunk ${i} failed`,
          );
        const pct = Math.round(((i + 1) / total) * 100);
        $("#upload-fill").style.width = pct + "%";
        $("#upload-progress-label").textContent = pct + "%";
      }
      const done = await api(`/api/ingest/${init.upload_id}/complete`, {
        method: "POST",
      });
    ...
    if (jobId != null) {
      pollJob(jobId, (j) => {
        $("#upload-status").textContent =
          `${Math.round(j.progress || 0)}% — ${esc(j.message || j.status)}`;
        if (j.status === "done") {
...
document.querySelector(".titlebar").addEventListener("dblclick", (e) => {
  if (window.desktop && !e.target.closest("button"))
...
window.addEventListener("mouseup", () => {
  if (sashDrag)
    localStorage.setItem("conductor.sidebarWidth", $("#sidebar").style.width);
  sashDrag = false;
...
let sashDragR = false;
sashREl.addEventListener("mousedown", (e) => {
  sashDragR = true;
  document.body.style.cursor = "col-resize";
  e.preventDefault();
});
window.addEventListener("mousemove", (e) => {
  if (!sashDragR) return;
  $("#right-pane").style.width =
    Math.max(240, Math.min(480, window.innerWidth - e.clientX)) + "px";
});
window.addEventListener("mouseup", () => {
  if (sashDragR)
    localStorage.setItem("conductor.rightWidth", $("#right-pane").style.width);
  sashDragR = false;
  document.body.style.cursor = "";
});
...
const BERNIE_NODE_TYPES = {
  ...
  text: {
    icon: "codicon-quote",
    label: "Text",
    defaultData: { text: "Hello from Text" },
  },
  json: {
    icon: "codicon-json",
    label: "JSON",
  ...
  http: {
    icon: "codicon-globe",
    label: "HTTP",
    defaultData: { url: "https://httpbin.org/get", method: "GET" },
  },
  ai: {
    icon: "codicon-chat-sparkle",
    label: "AI",
    defaultData: { prompt: "Summarize the input data" },
  },
  script: {
    icon: "codicon-code",
    label: "Script",
    defaultData: {
      script:
        "return { length: String(data && data.text ? data.text : JSON.stringify(data || {})).length }",
    },
  },
  sheet: {
    icon: "codicon-table",
    label: "Sheet",
    defaultData: { spreadsheetId: "", sheetName: "" },
  },
  drive: {
...
window.BernieNodeTypes = {
  register(type, cfg) {
    ...
    const meta =
      cfg.component ?
        {
          icon: cfg.icon || "codicon-extensions",
    ...
    if (cfg.component && !BERNIE_NODE_META[type])
      BERNIE_NODE_META[type] = {
        desc: cfg.desc || "Plugin node",
        color: cfg.color || "#8b5cf6",
      };
  },
  list() {
    return Object.entries(BERNIE_NODE_TYPES).map(([t, m]) => ({
      type: t,
      meta: m,
    }));
  },
...
const BERNIE_THEME_PRESETS = {
  ...
  sunset: {
    ...
    theme: {
      ...
      gradient: {
        primary: "linear-gradient(to right, #7f1d1d, #ea580c)",
        secondary: "none",
      },
      surface: "#7f1d1d",
      text: "#fffbeb",
  ...
  synthwave: {
    ...
    theme: {
      ...
      gradient: {
        primary: "linear-gradient(to bottom, #2a2139, #34294f)",
        secondary: "none",
      },
      surface: "#34294f",
      text: "#f97e72",
...
function bnLoadTheme() {
  try {
    const saved = JSON.parse(
      localStorage.getItem("conductor.bernie.theme") || "null",
    );
...
function bnLoadPanels() {
  try {
    const p = JSON.parse(
      localStorage.getItem("conductor.bernie.panels") || "null",
    );
    if (
      p &&
      (p.left === "flat" || p.left === "floating") &&
      (p.right === "flat" || p.right === "floating")
    )
      return p;
  } catch {
    /* */
...
function bnApplyTheme() {
  ...
  set(
    "--bn-grad-primary",
    t.gradient.primary === "none" ? t.background.primary : t.gradient.primary,
  );
  set(
    "--bn-grad-secondary",
    t.gradient.secondary === "none" ?
      t.background.secondary
    : t.gradient.secondary,
  );
  ...
  set(
    "--bn-radius",
    t.edges === "pill" ? "24px"
    : t.edges === "rounded" ? "12px"
    : "0px",
  );
  set("--bn-radius-inner", t.edges === "sharp" ? "0px" : "8px");
  set(
    "--bn-shadow",
    t.depth === "elevated" ? "0 8px 32px -8px rgba(0,0,0,0.8)"
    : t.depth === "shadow" ? "0 10px 30px -10px rgba(0,0,0,0.5)"
    : "none",
  );
  set(
    "--bn-padding",
    t.density === "compact" ? "0.5rem"
    : t.density === "spacious" ? "1.5rem"
    : "1rem",
  );
  const area = $("#bernie-canvas-area");
  if (area)
    area.style.background =
      t.gradient.primary !== "none" ? t.gradient.primary : t.background.primary;
  try {
    localStorage.setItem("conductor.bernie.theme", JSON.stringify(t));
  } catch {
    /* */
  }
  bnSyncPresetActive();
...
function bnSyncPresetActive() {
  ...
  $$("#bn-theme-presets .bn-preset").forEach((b) => {
    ...
    b.classList.toggle(
      "active",
      p.theme.background.primary === t.background.primary &&
        p.theme.highlight === t.highlight,
    );
  });
}
...
function bernieNodeSummary(n) {
  ...
  if (t === "sheet")
    return d.sheetName || d.spreadsheetId ?
        `Sheets ${d.spreadsheetId || ""}`.slice(0, 40)
      : "Google Sheet";
  if (t === "drive")
    return d.fileId ? `File ${d.fileId}`.slice(0, 40) : "Google Drive";
...
function bernieRenderCanvas() {
  ...
  for (const n of bernieState.nodes) {
    ...
    const node = el(
      ...
      "bnode " +
        statusCls(n) +
        (bernieView.selected.has(n.id) ? " bn-selected" : ""),
    );
    ...
    node.addEventListener("contextmenu", (e) => {
      e.preventDefault();
      if (!bernieView.selected.has(n.id)) {
        bernieView.selected.clear();
        bernieView.selected.add(n.id);
        bnSyncSelection();
      }
    ...
    node
      .querySelector(".bnode-edit")
      .addEventListener("click", () => bernieEditNode(n));
    node.querySelector(".bnode-del").addEventListener("click", () => {
      bernieState.nodes = bernieState.nodes.filter((x) => x.id !== n.id);
      bernieState.edges = bernieState.edges.filter(
        (x) => x.source !== n.id && x.target !== n.id,
      );
...
function bernieStartDrag(e, nodeEl, n) {
  ...
  const startX = e.clientX,
    startY = e.clientY;
  const origX = n.position?.x ?? 60,
    origY = n.position?.y ?? 60;
  ...
  const move = (ev) => {
    ...
    n.position = {
      x: Math.max(0, origX + (ev.clientX - startX) / z),
      y: Math.max(0, origY + (ev.clientY - startY) / z),
    };
    snap();
...
function bernieStartConnect(e, sourceId) {
  ...
  const move = (ev) => {
    ...
    const src = $("#bernie-canvas-inner").querySelector(
      `.bnode[data-id="${CSS.escape(sourceId)}"]`,
    );
    if (!src) return;
    const areaRect = $("#bernie-canvas-area").getBoundingClientRect();
    const z = bernieView.zoom || 1;
    const x1 = src.offsetLeft + src.offsetWidth,
      y1 = src.offsetTop + src.offsetHeight / 2;
    const x2 =
      (ev.clientX - areaRect.left + $("#bernie-canvas-area").scrollLeft) / z;
    const y2 =
      (ev.clientY - areaRect.top + $("#bernie-canvas-area").scrollTop) / z;
    const mx = (x1 + x2) / 2;
    temp.setAttribute(
      "d",
      `M ${x1} ${y1} C ${mx} ${y1}, ${mx} ${y2}, ${x2} ${y2}`,
  ...
  const up = (ev) => {
    ...
    if (targetEl) {
      ...
      if (
        targetId &&
        targetId !== sourceId &&
        !bernieState.edges.some(
          (x) => x.source === sourceId && x.target === targetId,
        )
      ) {
...
function bernieEditNode(n) {
  ...
  if (n.type === "text") {
    fields = field(
      "Text",
      `<textarea id="bn-f-text" rows="4">${esc(d.text || "")}</textarea>`,
    );
  } else if (n.type === "json") {
    fields = field(
      "JSON data",
      `<textarea id="bn-f-json" rows="8" spellcheck="false">${esc(d.jsonData || "{}")}</textarea>`,
    );
  } else if (n.type === "http") {
    ...
    fields = field(
      "Prompt (input data is passed in)",
      `<textarea id="bn-f-prompt" rows="5">${esc(d.prompt || "")}</textarea>`,
    );
  } else if (n.type === "script") {
    fields = field(
      "Script — `data` is the upstream output",
      `<textarea id="bn-f-script" rows="6" spellcheck="false">${esc(d.script || "")}</textarea>`,
    );
  } else if (n.type === "sheet") {
    fields = field(
      "Spreadsheet ID / name",
      `<input id="bn-f-sheet" value="${esc(d.spreadsheetId || "")}" placeholder="1A2b3C…" />`,
    );
  } else if (n.type === "drive") {
    fields = field(
      "File ID",
      `<input id="bn-f-drive" value="${esc(d.fileId || "")}" placeholder="1A2b3C…" />`,
    );
  } else {
    fields = `<div class="settings-note">${esc(meta.label)} node — ${
      n.type === "trigger" ? "starts the workflow with the data wired into it."
      : n.type === "custom" ?
        "integration hook; passes data through to the next node."
      : "passes data through to the next node."
    }</div>`;
  }
  openModal(
    `Configure ${meta.label || n.type} node`,
    `<div class="form-grid">
...
async function bernieExecuteNode(n, inputs) {
  ...
  try {
    ...
    if (n.type === "trigger") output = inputs;
      ...
      const res = await api("/api/bernie/proxy", {
        method: "POST",
        body: {
          url: d.url,
          method: d.method || "GET",
      ...
      const res = await api("/api/bernie/ai/execute", {
        method: "POST",
        body: { prompt: d.prompt, input_data: inputs },
      });
      output = res.result;
    } else if (n.type === "script") {
...
async function bernieRunWorkflow() {
  ...
  let starts = bernieState.nodes.filter(
    (n) => n.type === "trigger" && incoming.get(n.id) === 0,
  );
  if (!starts.length)
    starts = bernieState.nodes.filter((n) => incoming.get(n.id) === 0);
  ...
  while (queue.length) {
    ...
    ran.add(n.id);
    const upstream = bernieState.edges
    ...
    panel.innerHTML =
      flushLog.length ?
        flushLog
          .map(
            (f) =>
              `<div class="bernie-flush-item"><div class="view-title" style="font-size:0.75rem;margin:0 0 0.25rem">${esc(f.node)}</div><pre class="chat-code" style="margin:0;max-height:12rem;overflow:auto">${esc(typeof f.data === "string" ? f.data : JSON.stringify(f.data, null, 2))}</pre></div>`,
          )
          .join("")
...
async function bernieSave() {
  const payload = {
    name: bernieState.name || "Untitled canvas",
    nodes: bernieState.nodes,
    edges: bernieState.edges,
  };
  try {
    if (bernieState.canvasId) {
      await api(`/api/bernie/canvases/${bernieState.canvasId}`, {
        method: "PATCH",
        body: payload,
      });
    } else {
      const created = await api("/api/bernie/canvases", {
        method: "POST",
        body: payload,
      });
...
function bnGetLastCanvasId() {
  let raw = null;
  try {
    raw = localStorage.getItem(BERNIE_LAST_CANVAS_KEY);
  } catch {
    /* storage unavailable */
  }
  const n = Number(raw);
...
async function bnFetchCanvasList() {
  ...
  if (sel) {
    sel.innerHTML =
      ...
      bernieState.canvases
        .map(
          (c) =>
            `<option value="${c.id}">${esc(c.name)} (${c.node_count}n · ${c.edge_count}e)</option>`,
        )
        .join("");
...
async function bnLoadCanvas(id, opts = {}) {
  ...
  try {
    ...
    console.info(
      `[bernie] canvas load: ok (id=${c.id}, nodes=${bernieState.nodes.length}, edges=${bernieState.edges.length}, ${ms}ms)`,
    );
    if (!silent) toast(`Loaded "${c.name}"`, "ok");
    return true;
  } catch (err) {
    console.error(
      `[bernie] canvas load: failed (id=${id}):`,
      err?.message || err,
    );
    if (silent) throw err;
    toast(err.message, "err");
...
async function bernieOpenCanvas() {
  ...
  try {
    ...
    console.error(
      "[bernie] canvas-open: list fetch failed:",
      err?.message || err,
    );
    bnSetStageState("error", {
  ...
  const targetId =
    lastId && bernieState.canvases.some((c) => c.id === lastId) ?
      lastId
  ...
  try {
    ...
    bnSetStageState("error", {
      message: err?.message || `Could not load canvas ${targetId}.`,
    });
  }
}
...
function bnRenderLibrary() {
  ...
  if (!bernieState.canvases.length) {
    box.innerHTML =
      '<div class="bn-empty">No saved canvases yet — build a flow and hit Save.</div>';
    return;
  }
  box.innerHTML = bernieState.canvases
    .map(
      (c) => `
  ...
  box.querySelectorAll(".bn-lib-row").forEach((row) => {
    ...
    row
      .querySelector(".bn-lib-load")
      .addEventListener("click", () => bnLoadCanvas(id));
    row
      .querySelector('[data-act="dup"]')
      .addEventListener("click", () => bnDuplicateCanvas(c));
    row
      .querySelector('[data-act="del"]')
      .addEventListener("click", (e) => bnDeleteCanvas(c, e.currentTarget));
  });
}
...
function bernieAutoArrange() {
  ...
  for (const e of bernieState.edges)
    incoming.set(e.target, (incoming.get(e.target) || 0) + 1);
  const queue = bernieState.nodes.filter(
  ...
  for (const [d, list] of Object.entries(byDepth)) {
    list.forEach((n, i) => {
      n.position = { x: 60 + Number(d) * 260, y: 60 + i * 120 };
    });
  }
  bernieState.dirty = true;
  bernieRenderCanvas();
...
async function bernieSuggest() {
  ...
  if (panel)
    panel.innerHTML = '<div class="folder-loading">Asking the AI…</div>';
  bnOpenOutputRow();
  try {
    const res = await api("/api/bernie/ai/suggest", {
      method: "POST",
      body: { nodes: bernieState.nodes, edges: bernieState.edges },
    });
    if (panel) {
      panel.innerHTML = (res.suggestions || [])
        .map(
          (s) => `
        ...
        <div class="settings-note">${esc(s.description || "")}</div></div>`,
        )
        .join("");
      if (!(res.suggestions || []).length)
        panel.innerHTML =
          '<div class="settings-note">No suggestions returned.</div>';
    }
  } catch (e) {
    if (panel)
      panel.innerHTML = `<div class="settings-note">⚠ ${esc(e.message)}</div>`;
...
function bnZoomFit() {
  ...
  const z = Math.min(
    ...
    Math.max(
      0.3,
      Math.min(
        area.clientWidth / (maxX - minX + pad * 2),
        area.clientHeight / (maxY - minY + pad * 2),
      ),
    ),
  );
  bernieView.zoom = z;
...
function bnOpenMapFlyout() {
  ...
  if (willShow) {
    f.hidden = false;
    bnSyncMinimapControls();
  }
  void g;
}

function bnHideFlyouts() {
  const g = $("#bn-grid-flyout"),
    m = $("#bn-map-flyout");
  if (g) g.hidden = true;
  if (m) m.hidden = true;
}
...
function bnSyncGridControls() {
  ...
  set("bn-g-density", g.density);
  set("bn-g-snap-size", g.snapSize);
  set("bn-g-trans", g.transparency);
  set("bn-g-color", g.color);
  const show = $("#bn-g-show");
  if (show) show.checked = g.show;
  const snap = $("#bn-g-snap");
  if (snap) snap.checked = g.snap;
  $$("#bn-grid-flyout .bn-pattern-btn").forEach((b) =>
    b.classList.toggle("active", b.dataset.pat === g.variant),
  );
}
...
function bnSyncMinimapControls() {
  ...
  set("bn-m-opacity", m.opacity);
  set("bn-m-scale", m.scale);
  const show = $("#bn-m-show");
  if (show) show.checked = m.show;
}
...
function bnSyncSelection() {
  $$(".bnode").forEach((nEl) =>
    nEl.classList.toggle(
      "bn-selected",
      bernieView.selected.has(nEl.dataset.id),
    ),
  );
  ...
  if (bar) {
    ...
    if (cnt)
      cnt.textContent = `${bernieView.selected.size} node${bernieView.selected.size === 1 ? "" : "s"} selected`;
  }
}
...
function bnDuplicateSelected() {
  for (const id of [...bernieView.selected]) {
    ...
    copy.id = bnNewNodeId();
    copy.position = {
      x: (src.position?.x ?? 0) + 64,
      y: (src.position?.y ?? 0) + 64,
    };
    copy.status = "idle";
...
function bnSetPanelMode(side, mode) {
  ...
  if (btn) {
    ...
    btn.innerHTML =
      mode === "flat" ?
        '<span class="codicon codicon-panel-right"></span>'
      : '<span class="codicon codicon-replace-all"></span>';
  }
  try {
    localStorage.setItem(
      "conductor.bernie.panels",
      JSON.stringify(bernieView.panels),
    );
  } catch {
    /* */
  }
...
function bnRenderPalette() {
  ...
  if (!box) return;
  box.innerHTML = Object.entries(BERNIE_NODE_TYPES)
    .map(([t, m]) => {
...
function bnRenderThemePresets() {
  ...
  if (!box) return;
  box.innerHTML = Object.entries(BERNIE_THEME_PRESETS)
    .map(([key, p]) => {
      const t = p.theme;
      const active =
        t.background.primary === bernieView.theme.background.primary &&
        t.highlight === bernieView.theme.highlight;
...
function bnRenderThemeCustom() {
  ...
  const wireColor = (cid, tid, set) => {
    ...
    txt.addEventListener("change", () => {
      set(txt.value.trim() || "#000000");
      c.value = txt.value.trim() || "#000000";
      bnApplyTheme();
      bnRenderThemePresets();
    });
  };
  wireColor("bn-t-bgp-c", "bn-t-bgp-t", (v) => {
    bernieView.theme.background.primary = v;
  });
  wireColor("bn-t-bgs-c", "bn-t-bgs-t", (v) => {
    bernieView.theme.background.secondary = v;
  });
  wireColor("bn-t-sur-c", "bn-t-sur-t", (v) => {
    bernieView.theme.surface = v;
  });
  wireColor("bn-t-txt-c", "bn-t-txt-t", (v) => {
    bernieView.theme.text = v;
  });
  wireColor("bn-t-hl-c", "bn-t-hl-t", (v) => {
    bernieView.theme.highlight = v;
  });
  $("#bn-t-gp").addEventListener("change", (e) => {
    bernieView.theme.gradient.primary = e.target.value.trim() || "none";
    bnApplyTheme();
    bnRenderThemePresets();
  });
  $("#bn-t-gs").addEventListener("change", (e) => {
    bernieView.theme.gradient.secondary = e.target.value.trim() || "none";
    bnApplyTheme();
    bnRenderThemePresets();
  });
  $("#bn-t-trans").addEventListener("input", (e) => {
    bernieView.theme.transparency = e.target.value;
    $("#bn-t-trans-lbl").textContent = e.target.value + "%";
    bnApplyTheme();
    bnRenderThemePresets();
  });
  $("#bn-t-bright").addEventListener("input", (e) => {
    bernieView.theme.brightness = e.target.value;
    $("#bn-t-bright-lbl").textContent = e.target.value + "%";
    bnApplyTheme();
  });
  $("#bn-t-density").addEventListener("change", (e) => {
    bernieView.theme.density = e.target.value;
    bnApplyTheme();
  });
  $("#bn-t-edges").addEventListener("change", (e) => {
    bernieView.theme.edges = e.target.value;
    bnApplyTheme();
  });
  $("#bn-t-depth").addEventListener("change", (e) => {
    bernieView.theme.depth = e.target.value;
    bnApplyTheme();
  });
...
async function renderBernie(opts = {}) {
  ...
  /* ---- toolbar ---- */
  $("#bernie-name").addEventListener("input", (e) => {
    bernieState.name = e.target.value;
    bernieState.dirty = true;
  });
  ...
  $$(".bn-flyout-close").forEach((b) =>
    b.addEventListener("click", bnHideFlyouts),
  );
  $("#bn-grid-flyout #bn-g-show").addEventListener("change", (e) => {
    bernieView.grid.show = e.target.checked;
    bnApplyGrid();
  });
  $("#bn-grid-flyout #bn-g-density").addEventListener("input", (e) => {
    bernieView.grid.density = Number(e.target.value);
    $("#bn-g-density-lbl").textContent = e.target.value + "px";
    bnApplyGrid();
  });
  $("#bn-grid-flyout #bn-g-snap").addEventListener("change", (e) => {
    bernieView.grid.snap = e.target.checked;
  });
  $("#bn-grid-flyout #bn-g-snap-size").addEventListener("input", (e) => {
    bernieView.grid.snapSize = Number(e.target.value);
    $("#bn-g-snap-lbl").textContent = e.target.value + "px";
  });
  $("#bn-grid-flyout #bn-g-trans").addEventListener("input", (e) => {
    bernieView.grid.transparency = Number(e.target.value);
    bnApplyGrid();
  });
  $("#bn-grid-flyout #bn-g-color").addEventListener("input", (e) => {
    bernieView.grid.color = e.target.value;
    bnApplyGrid();
  });
  $$("#bn-grid-flyout .bn-pattern-btn").forEach((b) =>
    b.addEventListener("click", () => {
  ...
  $("#bn-map-flyout #bn-m-show").addEventListener("change", (e) => {
    bernieView.minimap.show = e.target.checked;
    bnRenderMinimap();
  });
  $("#bn-map-flyout #bn-m-opacity").addEventListener("input", (e) => {
    bernieView.minimap.opacity = Number(e.target.value);
    bnRenderMinimap();
  });
  $("#bn-map-flyout #bn-m-scale").addEventListener("input", (e) => {
    bernieView.minimap.scale = Number(e.target.value);
    bnRenderMinimap();
  });
  ...
  area.addEventListener("pointerdown", (e) => {
    ...
    if (
      e.target.closest(
        ".bnode, button, input, select, .bn-minimap, .bn-zoom-pill, .bn-flyout, .bn-panel, #bn-context-menu",
      )
    )
      return;
    bnHideFlyouts();
    ...
    if (bernieView.selectMode) {
      ...
      const x0 = e.clientX - sRect.left,
        y0 = e.clientY - sRect.top;
      mq.hidden = false;
      mq.style.left = x0 + "px";
      mq.style.top = y0 + "px";
      mq.style.width = "0px";
      mq.style.height = "0px";
      const move = (ev) => {
        const x1 = ev.clientX - sRect.left,
          y1 = ev.clientY - sRect.top;
      ...
      const up = (ev) => {
        ...
        const rect = area.getBoundingClientRect();
        const z = bernieView.zoom || 1;
        const l = Math.min(x0, ev.clientX - sRect.left),
          t = Math.min(y0, ev.clientY - sRect.top);
        const r = Math.max(x0, ev.clientX - sRect.left),
          b = Math.max(y0, ev.clientY - sRect.top);
        ...
        const hit = [];
        $$(".bnode").forEach((nEl) => {
          const x = nEl.offsetLeft,
            y = nEl.offsetTop;
          if (
            x < cx1 &&
            x + nEl.offsetWidth > cx0 &&
            y < cy1 &&
            y + nEl.offsetHeight > cy0
          )
            hit.push(nEl.dataset.id);
        });
      ...
      const up = () => {
        document.removeEventListener("pointermove", move);
        document.removeEventListener("pointerup", up);
        if (Math.abs(sl - area.scrollLeft) + Math.abs(st - area.scrollTop) > 2)
          bnClearSelection();
      };
      document.addEventListener("pointermove", move);
      document.addEventListener("pointerup", up);
  ...
  /* ---- zoom pill ---- */
  $("#bn-zoom-in").addEventListener("click", () =>
    bnSetZoom(bernieView.zoom * 1.2),
  );
  $("#bn-zoom-out").addEventListener("click", () =>
    bnSetZoom(bernieView.zoom / 1.2),
  );
  ...
  mm.addEventListener("pointerdown", (e) => {
    ...
    area.scrollLeft = Math.max(
      0,
      (e.clientX - rect.left) * kx - area.clientWidth / 2,
    );
    area.scrollTop = Math.max(
      0,
      (e.clientY - rect.top) * ky - area.clientHeight / 2,
    );
    bnRenderMinimap();
  });
  ...
  $("#bn-context-menu").addEventListener("click", (e) => {
    ...
    if (act === "inspect") bnInspect(t.id);
      ...
      bernieView.selected.clear();
      bernieView.selected.add(t.id);
      bnSyncSelection();
      bnDuplicateSelected();
    } else if (act === "delete") {
      bernieState.nodes = bernieState.nodes.filter((n) => n.id !== t.id);
      bernieState.edges = bernieState.edges.filter(
        (e) => e.source !== t.id && e.target !== t.id,
      );
      bernieState.dirty = true;
      bernieRenderCanvas();
  ...
  $("#bn-left-float").addEventListener("click", () =>
    bnSetPanelMode(
      "left",
      bernieView.panels.left === "flat" ? "floating" : "flat",
    ),
  );
  $("#bn-right-float").addEventListener("click", () =>
    bnSetPanelMode(
      "right",
      bernieView.panels.right === "flat" ? "floating" : "flat",
    ),
  );
...
const MODULE_STUBS = {
  content: {
    ...
    desc: "Content operations for listings, copy, and creative.",
    related: "catalog",
    relatedLabel: "Catalog",
  },
  case: {
    title: "Case Management",
    icon: "codicon-issue-opened",
    desc: "Customer and support cases across marketplaces.",
    related: "tasks",
    relatedLabel: "Action Queue",
  },
  fba: {
    title: "FBA",
    icon: "codicon-package",
    desc: "Fulfillment by Amazon — shipments, inventory, and prep.",
    related: "products",
    relatedLabel: "Products",
  },
  customerservice: {
    title: "Customer Service",
    icon: "codicon-person",
    desc: "Inbox, tickets, and customer communications.",
    related: "tasks",
    relatedLabel: "Action Queue",
  },
  brands: {
    ...
    desc: "Brand registry, brand health, and portfolio.",
    related: "products",
    relatedLabel: "Products",
  },
  people: {
    ...
    desc: "Team, roles, and the people behind the operation.",
    related: "asana",
    relatedLabel: "Asana",
  },
  listings: {
    ...
    desc: "Live listings and listing quality across channels.",
    related: "products",
    relatedLabel: "Products",
  },
  walmart: {
    title: "Walmart",
    icon: "codicon-globe",
    desc: "Walmart Marketplace operations.",
    related: "products",
    relatedLabel: "Products",
  },
  tiktok: {
  ...
  target: {
    title: "Target",
    icon: "codicon-globe",
    desc: "Target marketplace operations.",
    related: "products",
    relatedLabel: "Products",
  },
  spp: {
    title: "SPP",
    icon: "codicon-globe",
    desc: "SPP (Strategic Partner Platform) operations.",
    related: "products",
    relatedLabel: "Products",
  },
  coastal: {
    title: "Coastal",
    icon: "codicon-globe",
    desc: "Coastal channel operations.",
    related: "products",
    relatedLabel: "Products",
  },
  agency: {
    title: "Agency",
    icon: "codicon-globe",
    desc: "Agency client and brand management.",
    related: "products",
    relatedLabel: "Products",
  },
  agentbuilder: {
    title: "Agent Builder",
    icon: "codicon-rocket",
    desc: "Compose custom agents from skills and tools.",
    related: "agents",
    relatedLabel: "Agents",
  },
  runbooks: {
    title: "Runbooks",
    icon: "codicon-notebook",
    desc: "Step-by-step operational runbooks.",
    related: "sops",
    relatedLabel: "SOPs",
  },
  policies: {
    ...
    desc: "Company policies and governance documents.",
    related: "sops",
    relatedLabel: "SOPs",
  },
};
...
function renderModuleStub(view) {
  const m = MODULE_STUBS[view] || {
    title: view,
    icon: "codicon-circle",
    desc: "Module scoped — not yet built.",
    related: "",
    relatedLabel: "",
  };
  $("#view-root").innerHTML = `
...
function renderAmazon() {
  ...
  $("#view-root")
    .querySelectorAll(".home-card[data-nav]")
    .forEach((c) => {
    ...
    <div class="view">
      ...
      <div class="api-endpoints">
        ${[
          ...
          ["POST", "/webhooks/automation/{source}"],
          ["GET", "/api/data/table"],
          ["POST", "/api/data/pivot"],
          ["POST", "/api/svl/compare"],
          ["GET", "/api/flatfiles"],
        ]
          .map(
            ([m, p]) =>
              `<div class="api-row"><span class="method ${m.toLowerCase()}">${m}</span><span>${p}</span></div>`,
          )
          .join("")}
      </div>
...
async function renderWorkflows() {
  ...
  root
    .querySelector("#btn-run-brand-onboarding")
    .addEventListener("click", async () => {
      ...
      try {
        ...
        root.querySelector("#wf-cost-30").textContent =
          `$${res.forecasted_cost.day_30}`;
        root.querySelector("#wf-cost-60").textContent =
          `$${res.forecasted_cost.day_60}`;
        root.querySelector("#wf-cost-90").textContent =
          `$${res.forecasted_cost.day_90}`;
        root.querySelector("#wf-cost-total").textContent =
          `$${res.forecasted_cost.total_90d}`;
        ...
        toast(
          `Workflow complete! Created ${tasks.length} preview tasks.`,
          "info",
        );
      } catch (e) {
  ...
  root
    .querySelector("#btn-push-onboarding-asana")
    .addEventListener("click", async () => {
      toast("Pushing onboarding preview tasks to remote store...", "info");
      try {
        const res = await api("/api/workflows/push-onboarding-tasks", {
          method: "POST",
        });
...
async function loadModels(force = false) {
  ...
  try {
    ...
    const ggufRows = models
      ...
      .join("");
    const providerRows = (provs.providers || [])
      .map(
        (p) => `
      <div class="model-row">
...
function renderNavigationTab() {
  ...
  const presetOpts =
    Sidebar.SIDEBAR_PRESET_IDS.map(
      (id) =>
        `<option value="${id}" ${cfg.preset === id ? "selected" : ""}>${Sidebar.SIDEBAR_PRESETS[id].label}</option>`,
    ).join("") +
    ...
    .map((sec, si) => {
      const itemRows = (sec.items || [])
        .map((id, ii) => {
          ...
          const icon =
            item && item.icon ?
              item.icon.replace(/[^a-zA-Z0-9_-]/g, "")
            : "codicon-circle";
  ...
  const availOpts = available
    .map(
      (id) =>
        `<option value="${id}">${esc(Sidebar.NAV_ITEMS[id].label)}</option>`,
    )
    .join("");
  ...
  box.querySelector("#nav-preset").addEventListener("change", (e) => {
    ...
    cfg.sections = JSON.parse(
      JSON.stringify(Sidebar.SIDEBAR_PRESETS[e.target.value].sections),
    );
    commit();
  });
  box.querySelector("#nav-apply-preset").addEventListener("click", () => {
    ...
    cfg.preset = v;
    cfg.sections = JSON.parse(
      JSON.stringify(Sidebar.SIDEBAR_PRESETS[v].sections),
    );
    commit();
  });

  box.querySelectorAll(".nav-section-label").forEach((inp) =>
    inp.addEventListener("change", () => {
      const si = Number(inp.dataset.si);
      if (cfg.sections[si])
        cfg.sections[si].label = inp.value.trim() || "Section";
      Sidebar.saveSidebarConfig(cfg);
      Sidebar.renderSidebar();
    }),
  );
  ...
  box.querySelectorAll("[data-act]").forEach((b) =>
    b.addEventListener("click", () => {
      const si = Number(b.dataset.si),
        ii = Number(b.dataset.ii),
        act = b.dataset.act;
      const sec = cfg.sections[si];
      if (act === "sec-up" && si > 0) {
        [cfg.sections[si - 1], cfg.sections[si]] = [sec, cfg.sections[si - 1]];
      } else if (act === "sec-down" && si < cfg.sections.length - 1) {
        [cfg.sections[si + 1], cfg.sections[si]] = [
          cfg.sections[si],
          cfg.sections[si + 1],
        ];
      } else if (act === "sec-del") {
        cfg.sections.splice(si, 1);
      } else if (sec && act === "item-up" && ii > 0) {
        [sec.items[ii - 1], sec.items[ii]] = [sec.items[ii], sec.items[ii - 1]];
      } else if (sec && act === "item-down" && ii < sec.items.length - 1) {
        [sec.items[ii + 1], sec.items[ii]] = [sec.items[ii], sec.items[ii + 1]];
      } else if (sec && act === "item-del") {
  ...
  box.querySelector("#nav-add-item").addEventListener("click", () => {
    ...
    if (!last) {
      last = { label: "New Section", items: [] };
      cfg.sections.push(last);
    }
    last.items.push(id);
    commit();
  });
...
async function openFlatFileUpload() {
  ...
  try {
    presets = await api("/api/flatfiles/presets");
  } catch {
    /* presets optional */
  }
  ...
  $("#ff-up-go").addEventListener("click", async () => {
    ...
    try {
      const tpl = await api("/api/flatfiles/upload", {
        method: "POST",
        body: fd,
      });
      closeModal();
      toast(
        `Template "${tpl.name}" imported (${tpl.columns.length} columns)`,
        "ok",
      );
      refreshFlatFileList();
    } catch (e) {
...
async function refreshFlatFileList() {
  ...
  try {
    ...
    if (!tmpls.length) {
      ...
      const b = $("#ff-new2");
      if (b) b.addEventListener("click", () => openFlatFileEditor(null));
      return;
    }
    list.innerHTML = `<table class="data-table"><thead><tr><th>Template</th><th>Product type</th><th>Columns</th><th>Updated</th><th></th></tr></thead><tbody>${tmpls
      .map(
        (t) => `
      <tr>
        <td>${esc(t.name)}</td>
      ...
      .join("")}</tbody></table>`;
    list
      .querySelectorAll("[data-ff-edit]")
      .forEach((b) =>
        b.addEventListener("click", () =>
          openFlatFileEditor(Number(b.dataset.ffEdit)),
        ),
      );
    list
      .querySelectorAll("[data-ff-gen]")
      .forEach((b) =>
        b.addEventListener("click", () =>
          openFlatFileGenerate(Number(b.dataset.ffGen)),
        ),
      );
    list.querySelectorAll("[data-ff-del]").forEach((b) =>
      b.addEventListener("click", async () => {
    ...
    list.innerHTML = `<div class="empty-state">Failed: ${esc(e.message)}</div>`;
  }
}
...
async function openFlatFileEditor(id) {
  ...
  const ptOptions = (
    presets.product_types.length ?
      presets.product_types
    : [tpl.product_type])
    .map(
      (p) =>
        `<option value="${esc(p)}" ${tpl.product_type === p ? "selected" : ""}>${esc(p)}</option>`,
    )
    .join("");
  ...
  const renderCols = () => {
    const rows = (tpl.columns || [])
      ...
      .join("");
    $("#ff-cols").innerHTML =
      rows || '<div class="nav-empty">No columns — add below.</div>';
    $("#ff-cols")
      .querySelectorAll("[data-ff-col-del]")
      .forEach((b) =>
        b.addEventListener("click", () => {
          tpl.columns.splice(Number(b.dataset.ffColDel), 1);
          renderCols();
        }),
      );
  };
  ...
  openModal(
    id ? "Edit Flat-File Template" : "New Flat-File Template",
    `
  ...
  $("#ff-load-preset").addEventListener("click", () => {
    ...
    if (cols) {
      tpl.columns = JSON.parse(JSON.stringify(cols));
      renderCols();
      toast(`Loaded ${cols.length} columns for ${pt}`, "ok");
  ...
  $("#ff-save").addEventListener("click", async () => {
    const cols = Array.from(document.querySelectorAll("#ff-cols .ff-col-row"))
      .map((row) => ({
    ...
    const body = {
      ...
      product_type: $("#ff-pt").value,
      header_note: $("#ff-note").value.trim(),
      columns: cols,
    };
...
async function openFlatFileGenerate(id) {
  ...
  $("#ff-gen-do").addEventListener("click", async () => {
    const rows = Array.from(
      document.querySelectorAll("#ff-gen-rows .ff-gen-row"),
    ).map((r) => {
      const row = {};
      r.querySelectorAll(".ff-gen-val").forEach((inp) => {
        row[inp.dataset.key] = inp.value;
      });
      return row;
    });
    try {
      const res = await api(`/api/flatfiles/${id}/generate`, {
        method: "POST",
        body: { rows },
      });
      ...
      closeModal();
      toast(`Generated ${res.filename}`, "ok");
    } catch (e) {
      toast(`Generate failed: ${e.message}`, "err");
    }
  });
...
async function renderSvl() {
  ...
  $("#svl-text").addEventListener("keydown", (e) => {
    if ((e.ctrlKey || e.metaKey) && e.key === "Enter") runSvl();
  });
}
...
async function runSvl() {
  ...
  try {
    const data = await api("/api/svl/compare", {
      method: "POST",
      body: { suggested, field, threshold },
    });
    const best = data.best;
    const badge = (sim) =>
      sim >= 0.9 ? "pill-int pill-int-configured"
      : sim >= 0.6 ? "pill-int"
      : "pill-int pill-int-missing";
    res.innerHTML = `
      ...
      <tbody>${
        ...
        <tr>
          ...
          .join("") ||
        '<tr><td colspan="6" class="empty-state">No matches above threshold.</td></tr>'
      }</tbody></table>`;
  } catch (e) {
...
async function renderListings(tab = listingsState.tab) {
  ...
  root
    .querySelectorAll("[data-listing-tab]")
    .forEach((button) =>
      button.addEventListener("click", () =>
        renderListings(button.dataset.listingTab),
      ),
    );
...
async function renderLiveListings() {
  ...
  box.innerHTML =
    '<div class="folder-loading">Loading local listing index…</div>';
  try {
    const products = await window.ConductorData.get("products");
    if (!products.length) {
      box.innerHTML =
        '<div class="empty-state">No listings are in the local catalog yet. Import a catalog source first.</div>';
      return;
    }
    ...
    box
      ...
      .forEach((row) =>
        row.addEventListener("click", () =>
          renderProductDetail(Number(row.dataset.productId)),
        ),
      );
  } catch (e) {
    box.innerHTML = `<div class="empty-state">Could not load listings: ${esc(e.message)}</div>`;
  }
}
...
async function renderSuggestedVsLive() {
  ...
  $("#listing-upload").addEventListener("click", async () => {
    const file = $("#listing-suggested-file").files[0];
    if (!file) return toast("Choose a suggested content file first", "warn");
    const data = new FormData();
    data.append("file", file);
    results.innerHTML =
      '<div class="empty-state">Importing suggested content…</div>';
    try {
      const r = await fetch("/api/listings/suggested/upload", {
        method: "POST",
        body: data,
      });
      if (!r.ok) throw new Error(await r.text());
      const out = await r.json();
      listingsState.sourceId = out.source_id;
      results.innerHTML = `<div class="settings-note">Imported <b>${fmtNum(out.records_accepted)}</b> suggested listing records from ${esc(file.name)}. Refresh live data before comparison.</div>`;
    } catch (e) {
      results.innerHTML = `<div class="empty-state">Upload failed: ${esc(e.message)}</div>`;
    }
  });
  $("#listing-refresh").addEventListener("click", async () => {
    if (!listingsState.sourceId)
      return toast("Upload suggested content first", "warn");
    results.innerHTML =
      '<div class="empty-state">Refreshing stale listing snapshots…</div>';
    try {
      const out = await api("/api/listings/refresh", {
        method: "POST",
        body: { source_id: listingsState.sourceId },
      });
      results.innerHTML = `<div class="settings-note">${esc(out.status)} — ${fmtNum(out.refreshed || 0)} refreshed / ${fmtNum(out.stale || 0)} stale targets. ${out.status !== "success" ? "Configure Keepa or another live adapter to refresh stale records." : ""}</div>`;
    } catch (e) {
      results.innerHTML = `<div class="empty-state">Refresh blocked: ${esc(e.message)}</div>`;
    }
  });
  $("#listing-compare").addEventListener("click", async () => {
    if (!listingsState.sourceId)
      return toast("Upload suggested content first", "warn");
    results.innerHTML =
      '<div class="empty-state">Comparing suggested and fresh live content…</div>';
    try {
      const out = await api("/api/listings/compare", {
        method: "POST",
        body: { source_id: listingsState.sourceId, strict_fresh: true },
      });
      listingsState.report = out;
      results.innerHTML = `${out.stale_records ? `<div class="settings-note">${fmtNum(out.stale_records)} record(s) were excluded because live data is stale.</div>` : ""}<div class="data-table-wrap"><table class="data-table"><thead><tr><th>ASIN / SKU</th><th>Field</th><th>Suggested</th><th>Live</th><th>Similarity</th><th>Status</th><th>Recommendation</th></tr></thead><tbody>${
        out.rows
          .map(
            (r) =>
              `<tr><td class="mono">${esc(r.asin || r.sku)}</td><td class="mono">${esc(r.field)}</td><td>${esc(String(r.suggested).slice(0, 160))}</td><td>${esc(String(r.live).slice(0, 160))}</td><td>${(Math.max(r.levenshtein_similarity || 0, r.fuzzy_similarity || 0) * 100).toFixed(1)}%</td><td><span class="${
                r.match_status === "match" ? "pill-int pill-int-configured"
                : r.match_status === "near_match" ? "pill-int"
                : "pill-int pill-int-missing"
              }">${esc(r.match_status)}</span></td><td>${esc(r.recommendation || "—")}</td></tr>`,
          )
          .join("") ||
        '<tr><td colspan="7" class="empty-state">No fresh comparison rows. Refresh live data or use a configured adapter.</td></tr>'
      }</tbody></table></div>`;
    } catch (e) {
      results.innerHTML = `<div class="empty-state">Compare failed: ${esc(e.message)}</div>`;
    }
  });
}
...
async function renderListingGlossary() {
  const box = $("#listings-workspace");
  box.innerHTML =
    '<div class="folder-loading">Loading the local registry…</div>';
  try {
    const [glossary, spine] = await Promise.all([
      api("/api/spine/glossary"),
      api("/api/spine/snapshot"),
    ]);
    ...
    box.innerHTML = `<div class="empty-state">Registry unavailable: ${esc(e.message)}</div>`;
  }
}

async function renderSvl() {
  return renderListings("suggested");
}
...
async function renderBrandCompare() {
  ...
  $("#bc-brand").addEventListener("keydown", (e) => {
    if (e.key === "Enter") runBrandCompare();
  });
  ...
  try {
    ...
    $("#bc-brands").innerHTML = (meta.brands || [])
      .map((b) => `<option value="${esc(b)}">`)
      .join("");
    $("#bc-category").innerHTML =
    ...
    if ((meta.brands || []).length === 0) {
      $("#bc-results").innerHTML =
        `<div class="empty-state">No brands found in the catalog yet — import products (Catalog Ingest) to compare, or use the AI brief.</div>`;
    }
  } catch (e) {
    $("#bc-results").innerHTML =
      `<div class="empty-state">Could not load catalog meta: ${esc(e.message)}</div>`;
  }
}
...
function keepaCard(p) {
  ...
  const brandLine = [p.brand, p.manufacturer !== p.brand ? p.manufacturer : ""]
    .filter(Boolean)
    .join(" · ");
...
async function renderKeepa() {
  ...
  try {
    ...
    $("#keepa-domain").innerHTML = (st.domains || [])
      .map(
        (d) =>
          `<option value="${d.id}" ${d.id === st.domain ? "selected" : ""}>${esc(d.code)} (${d.id})</option>`,
      )
      .join("");
    const keyState = $("#keepa-key-state");
    keyState.textContent =
      st.has_key ? `configured ${st.key_masked}` : "not configured";
    keyState.className =
      "pill-int " + (st.has_key ? "pill-int-configured" : "pill-int-missing");
    if (!st.has_key)
      $("#keepa-key").placeholder = "No key yet — paste your Keepa API key";
  } catch (e) {
    toast(e.message, "err");
  }
  ...
  $("#keepa-save-key").addEventListener("click", async () => {
    ...
    try {
      await api("/api/keepa/config", {
        method: "POST",
        body: { api_key: key, domain },
      });
  ...
  $("#keepa-search-run").addEventListener("click", async () => {
    ...
    resEl.innerHTML =
      '<div class="empty-state">Searching Keepa products...</div>';
    try {
      const data = await api("/api/keepa/search", {
        method: "POST",
        body: { query, type, domain },
      });
  ...
  $("#btn-keepa-ai-query-run").addEventListener("click", async () => {
    ...
    try {
      const data = await api("/api/keepa/ai-query", {
        method: "POST",
        body: { prompt },
      });
      outBox.innerHTML = `<div>${data.summary ? data.summary.replace(/\n/g, "<br>") : "Query written"}</div>`;
    } catch (e) {
      outBox.innerHTML = `<span style="color:red;">Error: ${esc(e.message)}</span>`;
    }
  });
  ...
  const doLookup = async (refresh) => {
    ...
    res.innerHTML =
      '<div class="empty-state">Looking up live data on Keepa…</div>';
    try {
      const data = await api("/api/keepa/lookup", {
        method: "POST",
        body: { asins: keepaState.asins, domain, refresh },
      });
      keepaState.meta = data.meta || null;
      const mt = data.meta || {};
      $("#keepa-tokens").textContent =
        mt.tokensLeft != null ?
      ...
      if (data.notFound && data.notFound.length) {
        res.insertAdjacentHTML(
          "beforeend",
          `<div class="settings-note" style="margin-top:0.5rem">Not found on Keepa: ${esc(data.notFound.join(", "))}</div>`,
        );
      }
    } catch (e) {
      res.innerHTML = `<div class="empty-state">Failed: ${esc(e.message)}</div>`;
  ...
  $("#keepa-asins").addEventListener("keydown", (e) => {
    if (e.key === "Enter") doLookup(false);
  });
  ...
  try {
    ...
    $("#keepa-cached").innerHTML =
      `<div class="empty-state">Could not load cached products: ${esc(e.message)}</div>`;
  }
}

function wireKeepaImport(container) {
  container.querySelectorAll(".keepa-import").forEach((b) =>
    b.addEventListener("click", async () => {
      const asin = b.dataset.asin;
      const domain = Number(
        ($("#keepa-domain") && $("#keepa-domain").value) || keepaState.domain,
      );
      b.disabled = true;
      b.innerHTML = "Importing…";
      try {
        const r = await api("/api/keepa/import", {
          method: "POST",
          body: { asins: asin, domain },
        });
        if (r.imported.includes(asin))
          toast(`${asin} imported to catalog`, "ok");
        else if (r.updated.includes(asin))
          toast(`${asin} already in catalog — attributes updated`, "ok");
      ...
      b.disabled = false;
      b.innerHTML =
        '<span class="codicon codicon-cloud-upload"></span> Import to catalog';
    }),
  );
}
...
async function runBrandCompare() {
  ...
  res.innerHTML = '<div class="empty-state">Comparing…</div>';
  try {
    const data = await api("/api/brandcompare/compare", {
      method: "POST",
      body: {
        brand: brandCompareState.brand,
        category: brandCompareState.category,
        market: brandCompareState.market,
        limit: brandCompareState.limit,
      },
    });
...
function renderBrandCompareResult(d) {
  ...
  const columns = [
    {
      key: "__you__",
      label: `Your brand${you.brand ? ` — ${you.brand}` : ""}`,
      summary: you,
    },
  ...
  const head = `<tr><th class="bc-attr-head">Value attribute</th>${columns
    .map(
      (c) =>
        `<th>${esc(c.label)}<div class="bc-th-sub">${c.summary.product_count} products · ${c.summary.coverage}/${attrs.length} attrs</div></th>`,
    )
    .join("")}</tr>`;
...
async function runBrandBrief() {
  ...
  const res = $("#bc-results");
  res.innerHTML =
    '<div class="empty-state">Generating competitive brief…</div>';
  try {
    const data = await api("/api/brandcompare/brief", {
      method: "POST",
      body: { brand, category },
    });
    if (!data.ai) {
...
async function renderData() {
  ...
  try {
    ...
    $("#dm-source").innerHTML = srcs
      .map(
        (s) =>
          `<option value="${s.id}" ${s.id === dataState.source ? "selected" : ""}>${esc(s.label)} (${s.count})</option>`,
      )
      .join("");
    ...
    if (prod && prod.tags && prod.tags.length) {
      ...
      tagSel.innerHTML =
        ...
        prod.tags
          .map(
            (t) =>
              `<option value="${esc(t.tag)}">${esc(t.tag)} (${t.count})</option>`,
          )
          .join("");
      try {
  ...
  $("#dm-source").addEventListener("change", (e) => {
    dataState.source = e.target.value;
    dataState.selected.clear();
    $("#dm-tag").style.display =
      dataState.source === "products" && $("#dm-tag").options.length > 1 ?
        ""
      : "none";
    renderDataBody();
  });
  $("#dm-tag").addEventListener("change", async (e) => {
    ...
    dataState.selected.clear();
    try {
      await api("/api/settings/tagPref.data", {
        method: "PUT",
  ...
  $("#dm-q").addEventListener("input", (e) => {
    dataState.q = e.target.value;
  });
  $("#dm-q").addEventListener("keydown", (e) => {
    if (e.key === "Enter") renderDataBody();
  });
  root.querySelectorAll("[data-dm-mode]").forEach((b) =>
    b.addEventListener("click", () => {
      dataState.mode = b.dataset.dmMode;
      renderData();
    }),
  );
...
async function renderDataTable(body) {
  ...
  try {
    const tagParam =
      dataState.tag ? `&tag=${encodeURIComponent(dataState.tag)}` : "";
    const data = await api(
      `/api/data/table?source=${dataState.source}&limit=500&q=${encodeURIComponent(dataState.q)}${tagParam}`,
    );
    dataState.rows = data.rows;
    dataState.columns = data.columns;
    const cols = ["#", ...data.columns];
    const head = cols
      .map((c, i) =>
        i === 0 ? "<th></th>" : `<th data-dm-sort="${esc(c)}">${esc(c)}</th>`,
      )
      .join("");
    ...
    body.querySelectorAll(".dm-sel").forEach((cb) =>
      cb.addEventListener("change", () => {
        if (cb.checked) dataState.selected.add(cb.dataset.id);
        else dataState.selected.delete(cb.dataset.id);
      }),
    );
  } catch (e) {
    body.innerHTML = `<div class="empty-state">Failed: ${esc(e.message)}</div>`;
  }
}
...
async function renderDataPivot(body) {
  ...
  try {
    const srcs = await api("/api/data/sources");
    const src = srcs.find((s) => s.id === dataState.source) || {
      groupable: ["category"],
    };
    $("#dm-group").innerHTML = src.groupable
      .map(
        (g) =>
          `<option value="${esc(g)}" ${g === dataState.group_by ? "selected" : ""}>${esc(g)}</option>`,
      )
      .join("");
  } catch {
    /* */
  ...
  const run = async () => {
    dataState.group_by = $("#dm-group").value;
    dataState.agg = $("#dm-agg").value;
    const out = $("#dm-pivot-out");
    out.innerHTML = '<div class="empty-state">Aggregating…</div>';
    try {
      const res = await api("/api/data/pivot", {
        method: "POST",
        body: {
          source: dataState.source,
          group_by: dataState.group_by,
          agg: dataState.agg,
          measure: dataState.measure,
          q: dataState.q,
        },
      });
      const rows = res.rows || [];
      ...
      const bars = rows
        .map(
          (r) =>
            `<div class="dm-bar-row"><span class="dm-bar-label">${esc(r.key)}</span><div class="dm-bar-track"><div class="dm-bar-fill" style="width:${((Number(r.value) / max) * 100).toFixed(1)}%"></div></div><span class="dm-bar-val">${r.value}</span></div>`,
        )
        .join("");
      out.innerHTML = `
      ...
      out.innerHTML = `<div class="empty-state">Failed: ${esc(e.message)}</div>`;
    }
  };
...
async function renderDataWrangler(body) {
  ...
  try {
    const data = await api(
      `/api/data/table?source=${dataState.source}&limit=1000&q=${encodeURIComponent(dataState.q)}`,
    );
...
async function renderDataViews(body) {
  ...
  const load = async () => {
    try {
      ...
      body.querySelectorAll("[data-dv-load]").forEach((b) =>
        b.addEventListener("click", () => {
          dataState.mode = "table";
          renderData();
        }),
      );
      body.querySelectorAll("[data-dv-del]").forEach((b) =>
        b.addEventListener("click", async () => {
          await api(`/api/data/views/${b.dataset.dvDel}`, { method: "DELETE" });
...
async function exportDataCsv() {
  try {
    const data = await api(
      `/api/data/table?source=${dataState.source}&limit=2000&q=${encodeURIComponent(dataState.q)}`,
    );
    const cols = data.columns;
    const escCsv = (v) => {
      const s = String(v ?? "");
    ...
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `conductor-${dataState.source}.csv`;
    a.click();
    URL.revokeObjectURL(a.href);
  } catch (e) {
...
async function openAsanaPush() {
  let projects = [];
  try {
    projects = await api("/api/asana/projects");
  } catch {
    /* */
  }
  ...
  $("#ap-do").addEventListener("click", async () => {
    ...
    const rows = dataState.rows.filter((r) =>
      dataState.selected.has(String(r.id ?? r.sku ?? r.name)),
    );
    try {
      const res = await api("/api/data/push-asana", {
        method: "POST",
        body: {
          name,
      ...
      closeModal();
      if (res.executed === "live")
        toast(`Asana task created: ${res.detail}`, "ok");
...
function ftParseArgs(type, raw) {
  ...
  const args = {};
  if (type === "min_length") {
    const m = parseInt(raw.replace("min:", ""), 10);
    if (!isNaN(m)) args.min = m;
  } else if (type === "max_length") {
    const m = parseInt(raw.replace("max:", ""), 10);
    if (!isNaN(m)) args.max = m;
  } else if (type === "contains") {
    args.text = raw.replace("text:", "").trim();
...
function ftFieldOptions(cur) {
  ...
  return fields
    .map(
      (f) =>
        `<option value="${esc(f)}" ${cur === f ? "selected" : ""}>${esc(f)}</option>`,
    )
    .join("");
}
...
async function refreshFeatureList() {
  ...
  try {
    ...
    if (!feats.length) {
      ...
      const b = $("#ft-new2");
      if (b) b.addEventListener("click", () => openFeatureEditor(null));
      return;
    }
    list.innerHTML = `<table class="data-table"><thead><tr><th>Feature</th><th>Rating</th><th>Factors</th><th>Bands</th><th>Updated</th><th></th></tr></thead><tbody>${feats
      .map(
        (f) => `
      <tr>
      ...
      .join("")}</tbody></table>`;
    list
      .querySelectorAll("[data-ft-run]")
      .forEach((b) =>
        b.addEventListener("click", () => ftRun(Number(b.dataset.ftRun))),
      );
    list
      .querySelectorAll("[data-ft-push]")
      .forEach((b) =>
        b.addEventListener("click", () => ftPush(Number(b.dataset.ftPush))),
      );
    list
      .querySelectorAll("[data-ft-build]")
      .forEach((b) =>
        b.addEventListener("click", () => ftBuild(Number(b.dataset.ftBuild))),
      );
    list
      .querySelectorAll("[data-ft-edit]")
      .forEach((b) =>
        b.addEventListener("click", () =>
          openFeatureEditor(Number(b.dataset.ftEdit)),
        ),
      );
    list.querySelectorAll("[data-ft-del]").forEach((b) =>
      b.addEventListener("click", async () => {
        if (!confirm("Delete this feature?")) return;
        await api(`/api/features/${b.dataset.ftDel}`, { method: "DELETE" });
        toast("Feature deleted", "ok");
        refreshFeatureList();
      }),
    );
  } catch (e) {
...
async function ftRun(id) {
  ...
  try {
    const res = await api(`/api/features/${id}/run`, {
      method: "POST",
      body: {},
    });
    const rows = res.results
      ...
      <tr>
        ...
        <td><span class="${
          r.score < 60 ? "pill-int pill-int-missing"
          : r.score < 80 ? "pill-int"
          : "pill-int pill-int-configured"
        }">${r.score}</span></td>
    ...
    detail.innerHTML = `<div class="empty-state">Failed: ${esc(e.message)}</div>`;
  }
}

async function ftPush(id) {
  try {
    const res = await api(`/api/features/${id}/push-asana`, {
      method: "POST",
      body: {},
    });
    const live = res.live || 0;
    toast(
      `${res.total} task(s) → ${live} live, ${res.total - live} simulated`,
      live ? "ok" : "warn",
    );
  } catch (e) {
    toast(`Push failed: ${e.message}`, "err");
...
async function ftBuild(id) {
  try {
    const res = await api(`/api/features/${id}/build`, {
      method: "POST",
      body: {},
    });
    openModal(
    ...
    $("#ft-open-canvas").addEventListener("click", () => {
      closeModal();
      showView("bernie");
    });
  } catch (e) {
...
async function openFeatureEditor(id) {
  if (id) {
    ...
    featDraft = {
      name: "",
      description: "",
      rating_label: "Listing Quality Score (0-100)",
      factors: [],
      actions: [
        {
          min_score: 0,
          ...
          notes_template: "Score {score}/100. Failed: {failed_factors}",
        },
      ],
      source_report: "",
    };
  ...
  const renderFactors = () => {
    const rows = (featDraft.factors || [])
      ...
      .join("");
    $("#ft-factors").innerHTML =
      rows ||
      '<div class="nav-empty">No factors — add below or derive from a report.</div>';
    $("#ft-factors")
      .querySelectorAll("[data-ft-fdel]")
      .forEach((b) =>
        b.addEventListener("click", () => {
          featDraft.factors.splice(Number(b.dataset.ftFdel), 1);
          renderFactors();
        }),
      );
  };
  ...
  const renderBands = () => {
    const rows = (featDraft.actions || [])
      ...
      .join("");
    $("#ft-actions").innerHTML =
      rows || '<div class="nav-empty">No action bands — add one.</div>';
    $("#ft-actions")
      .querySelectorAll("[data-ft-bdel]")
      .forEach((b) =>
        b.addEventListener("click", () => {
          featDraft.actions.splice(Number(b.dataset.ftBdel), 1);
          renderBands();
        }),
      );
  };
  ...
  openModal(
    id ? "Edit Feature" : "New Feature from Report",
    `
  ...
  $("#ft-derive").addEventListener("click", async () => {
    ...
    try {
      const res = await api("/api/features/derive", {
        method: "POST",
        body: { report },
      });
      featDraft = res.spec;
      ...
      status.textContent =
        res.ai ?
          `Derived ${featDraft.factors.length} factors.`
        : `No AI — ${res.error || "built a blank template"}`;
    } catch (e) {
      status.textContent = `Derive failed: ${e.message}`;
    }
  });

  $("#ft-add-factor").addEventListener("click", () => {
    featDraft.factors = featDraft.factors || [];
    featDraft.factors.push({
      label: "",
      field: "name",
      type: "present",
      args: {},
  ...
  $("#ft-add-band").addEventListener("click", () => {
    ...
    featDraft.actions.push({
      min_score: 0,
      max_score: 60,
      project: "Catalog Ops",
      name_template: "Fix listing {sku}",
      notes_template: "",
    });
    renderBands();
  });
  ...
  $("#ft-save").addEventListener("click", async () => {
    const factors = Array.from(
      document.querySelectorAll("#ft-factors .ft-factor"),
    )
      .map((row) => ({
        ...
        args: ftParseArgs(
          row.querySelector(".ft-f-type").value,
          row.querySelector(".ft-f-args").value,
        ),
        weight: Number(row.querySelector(".ft-f-weight").value || 10),
      }))
      .filter((f) => f.field);
    const actions = Array.from(
      document.querySelectorAll("#ft-actions .ft-factor"),
    ).map((row) => ({
