...
async function health() {
  ...
  try {
    ...
    console.log(
      `health: ${res.ok ? "OK" : "FAIL"} (${res.status}) —`,
      JSON.stringify(body),
    );
    return res.ok ? 0 : 1;
  } catch (e) {
...
async function plugins(action, id) {
  ...
  try {
    if (action === "enable" || action === "disable") {
      if (!id) {
        console.error("plugins: --enable/--disable requires a plugin id");
        return 1;
      }
      ...
      if (!res.ok) {
        console.error(`plugins: ${res.status} ${await res.text()}`);
        return 1;
      }
    ...
    for (const p of data.plugins || []) {
      console.log(
        `${p.enabled ? "✓" : "✗"} ${p.id.padEnd(16)} v${p.version}  ${p.name || ""}`,
      );
    }
    return 0;
  } catch (e) {
    console.error(
      `plugins: backend unreachable on :${port} — ${e.message} (start the app first)`,
    );
    return 1;
  }
}
...
function doctor() {
    ...
    for (const dep of ["fastapi", "uvicorn", "multipart", "requests"]) {
      try {
        ...
        check(
          `dep ${dep}`,
          false,
          "missing — run pip install fastapi uvicorn python-multipart requests",
        );
      }
    }
  }
  ...
  if (existsSync(keyFile)) {
    try {
      ...
      check(
        "provider keys",
        true,
        `${Object.keys(keys).length} stored, ${encrypted.length} encrypted`,
      );
    } catch {
      check("provider keys", false, "unparseable provider-keys.json");
    }
  }
  ...
  check(
    "dist artifacts",
    distExes.length > 0,
    distExes.join(", ") || "run npm run dist",
  );
  return ok ? 0 : 1;
}
...
function checksums() {
  if (!existsSync(DIST)) {
    console.error("checksums: dist/ missing — run the build first");
    return 1;
  }
  const exes = readdirSync(DIST)
    ...
    .sort();
  if (!exes.length) {
    console.error("checksums: no .exe artifacts in dist/");
    return 1;
  }
  ...
  for (const f of exes) {
    const p = join(DIST, f);
    console.log(
      `${sha256(p)}  ${f}  (${(statSync(p).size / 1048576).toFixed(1)} MB)`,
    );
  }
  return 0;
}
...
function release() {
  ...
  try {
    console.log("release: building one x64 NSIS installer…");
  ...
  const cs = checksums();
  console.log(
    "release: staged under dist/ — upload the single Setup installer and checksums above.",
  );
  return cs;
}
...
switch (cmd) {
  case "health":
    ...
    break;
  case "plugins":
    process.exit(await plugins(flag?.replace("--", ""), id));
    break;
  case "doctor":
    process.exit(doctor());
    break;
  ...
  release               build one x64 NSIS installer, then print checksums`);
